...
### 🗄️ Order Studio (ТЗ) — убран в «дальний ящик» (физический перенос)
- Весь код раздела ТЗ физически перенесён в `pinhead-react/src/orderstudio/**`
  (97 файлов, git-renames, зеркальная структура). ERP + shared остались в `src/`.
  Импорты пересчитаны детерминированным кодмодом (транзитивное замыкание от ERP-корней).
- Правило заморозки — в CLAUDE.md (корневой + pinhead-react): `src/orderstudio/**`
  не читать/не трогать/не тестировать без явной команды (экономия токенов).
- `main.jsx`: ТЗ-store (`useStore`) грузится динамическим `import()` → полностью вне
  корневого бандла ERP (в билде отдельные чанки OrderStudioApp/useStore).
- Общими осознанно оставлены модули ERP-мостика «Заказы ТЗ»: `AdminPanel`, `PageHeader`,
  `useOrdersStore`, `data/*` (каталоги), `validate`, `types/{order,catalog}`.
- Проверено: build ✓, lint ✓, 953 unit-теста ✓. Поведение рантайма не изменилось
  (ТЗ по-прежнему включается `?studio=1`).
