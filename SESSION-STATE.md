...
## Состояние на 2026-07-22 (сессия 19) — Правка 4.1.3: авто-заполнение плана при приёмке материалов

**Замысел:** плановые данные материала вносит закупщик (этап «Закупка») и они read-only приходят
в задачу склада «Приёмка материалов». Кладовщик вносит только ФАКТ и сверяет план/факт — ручное
дублирование плана устранено. Ветка `warehouse-auto-fill-plan` = **только 4.1.3**.

**Ключевая находка:** закупка и приёмка работают с ОДНОЙ строкой `erp_materials`, поэтому
«авто-подтягивание» плана — это просто read-only показ полей плана в приёмке (отдельная сущность
закупки не нужна). Разделены графы план/факт.

**Модель (`20260722160000_erp_material_plan_fact.sql`):** `erp_materials` += `color`/`article`
(план закупки) и `name_actual`/`color_actual`/`article_actual` (факт склада); `qty_expected`
(план, кг) и `qty_received` (факт, кг) — уже были. Все новые поля nullable → бэк-совместимо.

**Гейт «без плана сделка не пойдёт дальше»:** плановое кол-во (кг) обязательно для `source='purchase'`.
Форма закупки требует его при добавлении; `maybeCloseSupply` не закрывает «Закупку», пока у закупаемого
материала нет `qty_expected` (этап supply не done → задача приёмки не создаётся). toast-подсказка.

**UI:**
- Закупка (`FabricPurchasing`): поля Цвет/Артикул/Плановое кол-во (кг, обязательно для закупки);
  в таблице колонка «План, кг» + цвет/артикул подписью под материалом.
- Приёмка (`MaterialReceiptCard`): переработана в два блока — **План** (read-only, из закупки) и
  **Факт** (ввод кладовщика) + таблица сверки «Поле · План · Факт» с подсветкой расхождений и дельтой
  по кол-ву. `acceptMaterial` пишет только факт; `onUpdateMaterial` из карточки убран (план read-only).

**Проверки:** 995 unit-тестов (3 новых: гейт плана ×2, сохранение факта), lint 0, build ok.
**⚠️ Деплой:** миграция `20260722160000` НЕ применена к удалённой БД — применить при деплое
(`supabase db push` / MCP). Колонки additive-nullable → безопасно.
**Следующее (не сделано):** остальные пункты ТЗ — 4.2.1 (приёмка готового изделия от подрядчика
складом), 4.2.2 (не создавать закупку для материалов подрядчика), 4.2.3 («отдельная операция» +
маршрут возврата), 4.2.4 + редизайн frontend-концепции — отдельные задачи.
