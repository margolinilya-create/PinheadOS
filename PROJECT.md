...
### Сессия 16 (20.07.2026) — Order Studio (ТЗ) в «дальний ящик»

- Физический перенос всего кода раздела **ТЗ (Order Studio)** в `src/orderstudio/**`
  (97 файлов, git-renames, зеркальная структура; импорты пересчитаны кодмодом). ERP и
  shared-инфраструктура остались в `src/`. Цель — заморозить старый проект ТЗ, чтобы не
  тратить на него внимание/токены в ERP-сессиях.
- `main.jsx` переведён на динамический импорт ТЗ-store → ТЗ полностью вне корневого бандла
  (отдельные lazy-чанки OrderStudioApp/useStore).
- Граница ТЗ/shared вычислена транзитивным замыканием от ERP-корней; мостик «Заказы ТЗ»
  (`AdminPanel` + `useOrdersStore` + `data/*` + `PageHeader` + `validate` + `types/{order,catalog}`)
  осознанно оставлен общим.
- Правило заморозки закреплено в CLAUDE.md (корневой + pinhead-react). build ✓, lint ✓, 953 теста ✓.
