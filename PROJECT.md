...
### Сессия 10 (11.04.2026) — мобильная адаптация SKU-редактора

**Mobile readability (#90):**
- SKU Items таблица: скрыты неключевые колонки на ≤768px (№, фото, артикул, расход отд., зоны) и ≤480px (fit, отделка)
- Fabrics таблица: скрыты колонки состав, плотность, «используется в» на ≤768px; поставщик на ≤480px
- Убран `min-width: 900px` на группах — таблица не заставляет скроллить горизонтально
- `table-layout: fixed` → `auto` на мобильных для гибкой ширины колонок
- Тулбар: стекается вертикально, поиск на всю ширину
- Supplier filter: горизонтальный скролл без scrollbar
- Компактные отступы, шрифты, заголовки групп
- Модалки адаптированы под мобильный экран
- Rate bar, extras editor, category tabs — адаптированы
- Правила column-hiding scoped через `.sku-items-view` — не затрагивают Hardware tab
