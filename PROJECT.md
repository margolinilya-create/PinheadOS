...
### Сессия 23 (28.07.2026) — Доводка ERP: дизайн-система, UX цехов, недостроенный функционал

Разведка 3 Explore-агентами показала, что редизайн (сессия 20) заявлен как завершённый, но
исполнение отставало: иконок не было вовсе, токены не применялись, примитивов не существовало.

- **Иконки.** `erp/components/icons.js` — собственный линейный набор 34 иконок 24×24 как данные
  (без JSX, без npm-зависимостей) + `Icon.jsx`. Убраны все 258 эмодзи из 54 файлов: шапка, меню,
  KPI, кнопки цеха, канбан, склад, закупка.
- **Дизайн-фундамент.** Токены `--control-h*` (на `pointer: coarse` растут до 44px сами),
  `--focus-ring`, `--shadow-pop/drawer`, `--radius-xl`; 82 хардкод-отступа → `var(--space-*)`.
  Примитивы `Button` / `Field` / `EmptyState` / `ErrorState` со своими CSS-модулями.
- **Экран цеха.** Форма брака (до 12 полей внутри карточки) → пошаговый `DefectWizard` в боковой
  панели. Состав payload у `onDefect` зафиксирован тестом. Ряд действий крупный (48px на тач).
- **Производительность.** `React.memo` на элементах списков + `useCallback` на DnD канбана
  (без него memo бесполезен). `100vh` → `100dvh`. Видимый `:focus-visible` на всей оболочке.
- **Надёжность.** Промис-синглтон загрузки каталогов (было до 4 параллельных запросов на старте),
  дедупликация `fetchOrders`, `ErrorBoundary` вокруг ERP-роутов с `key={pathname}`.
- **Функционал.** Гейт отгрузки теперь судит о материалах тем же правилом, что и гейт этапа
  (`isMaterialPending`) + `shipBlockReason` с текстом причины; новый экран **«Загрузка цехов»**
  (`/load`, сетка «цех × день» из плановых дат); исполнитель задачи закупки из UI.

**Проверка:** lint 0, **1090 unit** (было 1018), build ок; экраны сняты headless-Playwright
(дашборд, очередь цеха, мастер брака оба шага, `/load`).

**Не сделано осознанно:** мощности цехов в `/load` — колонок нет, их удалила миграция
20260716170000 решением «мощности не используются»; эталоны visual локально не перегенерены
(локальный Chromium 1194 ≠ CI 1208, дрейфует даже нетронутый `studio-sku`) — обновлять
self-heal-диспатчем CI.
