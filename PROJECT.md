...
### Сессия 20 (22.07.2026) — Редизайн фронтенда, PR1 (оболочка) + мёрдж правок 4.x

- **Мёрдж на прод:** правки 4.1.3 + 4.2.1–4.2.4 (PR #123, squash `1cc5047`; CI `test` зелёный).
- **Старт трека редизайна по макетам заказчика + UI Kit.** PR1 — фундамент/оболочка (только frontend):
  - Палитра из UI Kit (primary `#2563EB`, статусные цвета, светлый фон/белые карточки) как
    **ERP-scoped** токены на корне `.shell` — Order Studio не затронут, тёмная тема сохранена.
  - Вертикальный сгруппированный `Sidebar` (`erp/layout/Sidebar.jsx`) со счётчиками активных задач
    по разделам и сворачиванием в иконочный рельс; обновлённый Header; раскладка Sidebar | (Header+Main).
  - Компонент `Badge` (`erp/components/Badge.jsx`) + перекраска `.chip*` (в работе=синий, ожидает=амбер).
  - Count-хелперы в `orderHelpers.ts`. 1006 тестов (+5), lint/build чисто; визуальная проверка
    (свет/тьма/свёрнутый) через Playwright.
  - Дальше: Dashboard → таблицы+FilterBar+Pagination → карточка-Drawer+Stepper → Подряд/Эксперимент.
