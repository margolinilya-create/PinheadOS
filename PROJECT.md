...
### Сессия 19 (22.07.2026) — Правка 4.1.3: авто-заполнение плана при приёмке материалов

- **Замысел:** план материала вносит закупщик, склад видит его read-only и вносит только факт;
  ручное дублирование плана кладовщиком устранено, план/факт можно сверить.
- **Модель:** `erp_materials` += `color`/`article` (план закупки), `name_actual`/`color_actual`/
  `article_actual` (факт склада); миграция `20260722160000_erp_material_plan_fact.sql` (nullable, additive).
  `qty_expected`/`qty_received` (план/факт кол-ва, кг) — уже были.
- **Закупка (`FabricPurchasing`):** плановое кол-во (кг) стало обязательным для `source='purchase'`
  + поля Цвет/Артикул; гейт `maybeCloseSupply` не закрывает «Закупку» без планового кол-ва
  («сделка не пойдёт дальше»).
- **Приёмка (`MaterialReceiptCard`):** переработана в два блока — **План** (read-only из закупки) и
  **Факт** (ввод кладовщика: материал/цвет/артикул/принято-кг/статус/комментарий) + таблица сверки
  план/факт с подсветкой расхождений и дельтой по кол-ву. `acceptMaterial` пишет только факт.
- **Тесты:** 995 unit (3 новых). Lint 0, build ok. Скоуп ветки — только 4.1.3; 4.2.x и редизайн —
  отдельные задачи. Миграцию применить при деплое.
