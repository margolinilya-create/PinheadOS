...
### ✅ Исправлено (сессия 23)
- **Материалы `packaging`/`other` и «сиротские»** — `isOrderReadyToShip` теперь судит о материалах
  тем же правилом, что и гейт этапа (`isMaterialPending` вынесен из `routes.ts`): пришедший
  закупочный материал годен только после приёмки складом. Причина показывается через
  `shipBlockReason(order)` в карточке заказа, боковой панели и задаче склада.
- **Inline-состояние ошибки загрузки** — компонент `ErrorState` («Не удалось загрузить» +
  «Повторить») подключён на Закупке, Складе, Подряде, Производстве и в Эксперим. цехе.
- **Исполнитель задачи закупки** назначается из UI (инлайн-правка `responsible` в таблице дозакупок).

### ℹ️ Оказались неактуальными (проверено в коде, сессия 23)
- **Возврат на ранний этап** промежуточные этапы переоткрывает — `stagesSlice.ts:144-161`.
- **Закрытие задачи закупки не переоткрывает этап-источник** — гейт `isStageAwaitingProcurement`
  вычисляемый (смотрит на статус задачи вживую), этап разблокируется сам.
