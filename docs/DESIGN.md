...
**Иконки — свои, без зависимостей.** `erp/components/icons.js` хранит набор из 34 линейных иконок
...
Стрелка `→` в текстах — типографика, остаётся.

**Метрики контролов — токенами.** На `.shell`: `--control-h-sm` / `--control-h` / `--control-h-lg`,
в `@media (pointer: coarse)` → 40/44/48px. Правило «тач-таргет ≥44px» выполняется само, без правок
по месту. Плюс `--focus-ring`, `--shadow-pop`, `--shadow-drawer`, `--radius-xl`.
...
| `EmptyState` / `ErrorState` | `components/States.jsx` | иконка + заголовок + пояснение + действие; ошибка — `role="alert"` + «Повторить» |
