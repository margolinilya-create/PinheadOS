...
| Очередь цеха (карточка) | `queueCard` + полоса слева (Ready зел./Progress жёлт./Blocked красн.), просрочка — красная рамка | мобильный вид (<760px): кнопки min-height 46px (планшет); блок «📋 ТЗ позиции»; превью 48px + лайтбокс; прогресс-бар qty_done N/M; фото брака/блока |
...
| Страница задания | `taskGrid` (2 колонки) + `taskFacts` (dl) | `/task/:stageId`: факты задания слева, маршрут справа, ниже ТЗ+действия, файлы, комментарии; номер заказа — ссылка |
