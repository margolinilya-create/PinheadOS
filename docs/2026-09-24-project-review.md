...
| 7–15 | остальное | не начато, порядок ниже |
...
**7. Наблюдаемости нет.** `docs/OPERATIONS.md`: «узнать, что сломалось,
можно только по звонку из цеха». `ErrorBoundary` пишет в консоль пользователя.
→ Без новой зависимости: `ErrorBoundary` и `window.onerror` → INSERT
в `erp_client_errors` (RLS: insert для `authenticated`, select — admin),
плитка в админке. Sentry — отдельным решением владельца (npm-зависимость).
...
**10. 4150 строк сгенерированных типов лежат мёртвым грузом.**
`src/types/database.generated.ts` есть, но `lib/supabase.ts` создаёт клиент
без `<Database>`; `erp/types.ts` (2396 строк) ведётся руками, и `schema.test.ts`
существует именно потому, что они уже расходились. 216 компонентов `.jsx`,
0 `.tsx`, `checkJs: false` — компонентный слой не типизируется вовсе.
→ `createClient<Database>()`; новые компоненты — `.tsx`; старые — по касанию.
