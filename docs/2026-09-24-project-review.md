...
| № | Находка | Статус |
|---|---|---|
| 1 | Бэкапа Storage нет | секреты **заданы** (прогон #58 владельца, 24.09 22:41 UTC: шаг секретов зелёный). ⚠️ Сессия 68: репозиторий публичный — лог с путями объектов и архив бакета (~401 МБ) видны всем; workflow в нынешнем виде запускать нельзя |
...
| 4 | Leaked password protection выключен | **исправлено** в сессии 68: `password_hibp_enabled = true` через Management API, advisor пуст |
...
Password security.
→ **Сделано 24.09 (сессия 68)**: `password_hibp_enabled = true` через
Management API, повторное чтение подтвердило, advisor исчез из списка.
...
и `.claude-flow/` — классификатор сессии отказал в необратимом удалении;
после снятия хуков файлы ни на что не ссылаются и безопасны к удалению
одной командой `git rm -r`.
...
1. **За владельцем, без кода (полчаса):** секреты бэкапа (1), защита `main`
   + гейт деплоя Vercel (3), тумблер паролей (4), `git rm -r .claude/helpers
   .claude/proven-config.json .claude/.proven-config-version` (6).
