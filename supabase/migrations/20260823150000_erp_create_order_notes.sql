-- Создание заказа: секция «Заметки к заказу» (правка 22.08, п. 5.8).
--
-- Текст функции — тот, что применён к базе миграцией 20260823120000, плюс
-- вставка `erp_order_notes` и адресация файлов заметок по `note_index`.
-- Пересоздаётся ЦЕЛИКОМ по подлинному тексту: `create or replace` иначе
-- не умеет, а писать по памяти правило проекта запрещает прямо.
