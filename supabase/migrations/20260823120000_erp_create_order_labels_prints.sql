-- Создание заказа: бирки позиции и привязка макетов к нанесениям
-- (правки заказчика 22.08, пункты 5.2–5.3).
--
-- Текст функции взят ПОДЛИННЫЙ — `pg_get_functiondef` из боевой базы, а не
-- из файла репозитория: правило проекта, и однажды оно уже спасло связь
-- заказа с ТЗ. Заголовок приведён к виду миграций (`create or replace
-- function public.…`, `$$`), потому что `pg_get_functiondef` печатает его
-- заглавными и с `$function$` — скопированный дословно, он молча уводит
-- сторожевые тесты на предыдущую миграцию.
--
-- ЧТО ДОБАВЛЕНО. Три вещи, и все — теми же приёмами, что уже есть в функции:
--
-- 1. `main_fabric` в позицию (п. 5.1) — просто ещё одна колонка техблока.
--
-- 2. Секция `labels` внутри позиции (п. 5.3): бирки вставляются как нанесения,
--    с накоплением `v_label_ids` — по тому же образцу, что `v_mat_ids`.
--
-- 3. `print_index` и `label_index` у вложений (пп. 5.2–5.3). Строк нанесений
--    и бирок на момент выбора файла ещё НЕ СУЩЕСТВУЕТ — заказ создаётся одной
--    транзакцией, — поэтому файл адресуется НОМЕРОМ внутри позиции, ровно как
--    `stage_index` у файлов подрядного шага. И ключ так же составной
--    («позиция:нанесение»): `v_print_ids` обнуляется на каждой позиции,
--    и голый индекс нанесения указывал бы на чужой макет.
--
--    Номер проставляет ТОТ ЖЕ код, который строит секции `prints`/`labels`, —
--    иначе рядом заведётся второй порядок, и он разойдётся с первым.
...
begin
  ...
  loop
    ...
    insert into erp_order_attachments
      (order_id, item_id, material_id, stage_id, print_id, label_id,
