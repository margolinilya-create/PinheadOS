-- Правки заказчика 07.09, п. 4: «Готовое изделие» — два сценария.
--
-- ЧТО ПРОСИТ ДОКУМЕНТ. «Разделить тип производства „Готовое изделие“ на два
-- сценария»; детализацию документ откладывает, и ось назвал владелец в этой
-- сессии: ЧЬЁ ИЗДЕЛИЕ.
--
--   `customer`  — давальческое: изделие привозит клиент, мы только наносим.
--                 Закупать нечего, а приёмка на складе обязательна.
--   `purchased` — изделие закупаем мы: лист закупки и этап «Закупка».
--
-- ПОЧЕМУ КОЛОНКА У ПОЗИЦИИ, А НЕ У ЗАКАЗА. Механизм «закупать нечего» в проекте
-- уже есть — `erp_orders.purchase_required`, — но он про ВЕСЬ заказ, а на бое
-- один заказ уже несёт ДВЕ позиции `ready_garment`: сценарий у них может быть
-- разный, и отметка на заказе сняла бы закупку у обеих разом.
--
-- ПОЧЕМУ НЕ `material_source`. Та колонка отвечает на вопрос ПОДРЯДА («чей
-- материал у подрядчика»: pinhead / contractor / mixed) и в маршруте читается
-- только при `production_type = 'outsource'`. Ответ «изделие клиента» —
-- другая величина; сведённые в одну колонку, они разошлись бы в первую же
-- правку, причём молча.
--
-- NULL ЧИТАЕТСЯ КАК «ЗАКУПАЕМ МЫ» (клиент сравнивает строго с 'customer',
-- как с `executor = 'contractor'`). Пять заведённых позиций `ready_garment`
-- остаются с прежним маршрутом — бэкфилл не нужен и был бы догадкой о том,
-- чьё изделие в уже сданных заказах.

alter table public.erp_order_items
  add column if not exists garment_source text;
...
alter table public.erp_order_items
  ...
  check (garment_source is null or garment_source in ('customer', 'purchased'));

comment on column public.erp_order_items.garment_source is
  'Чьё готовое изделие (правки 07.09, п. 4): customer — давальческое, изделие клиента (закупки нет, приёмка склада обязательна); purchased — закупаем мы. NULL — вопрос не задавался (позиция не «Готовое изделие» либо заведена до правки), читается как purchased.';

-- ── Создание заказа пишет сценарий ──────────────────────────────────────────
--
-- Функция пересоздана ЦЕЛИКОМ по подлинному тексту: тело файла
-- `20260907180055_erp_packaging_mm_print_effect_garment_kind.sql` сверено
-- с `prosrc` живой функции ДО правки — md5 совпал побайтово
-- (53b8ab85c28f8c04630fd842acc83017, 12333 символа). Добавлена ровно одна
-- вставка: `garment_source` в INSERT позиции. Подстановки проверены
-- на однозначность при сборке файла: не сработавшая замена оставила бы
