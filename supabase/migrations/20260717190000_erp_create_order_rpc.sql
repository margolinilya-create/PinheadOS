-- RPC erp_create_order(payload jsonb) → uuid: атомарное создание
-- производственного заказа в ОДНОЙ транзакции (заказ + позиции + нанесения +
-- этапы с depends_on + материалы). Заменяет 4+ последовательных insert
-- и N+1 update depends_on из useErpStore.createOrder (п.28).
--
-- Маршрут (этапы и зависимости) считается на клиенте (buildRoute) и передаётся
-- готовым: stages[].depends_on — 0-based индексы этапов ТОЙ ЖЕ позиции
-- (buildRoute гарантирует, что зависимости указывают на более ранние этапы).
--
-- SECURITY INVOKER: RLS erp_* уже разрешает insert любому authenticated,
-- поэтому DEFINER не нужен — функция работает под правами вызывающего.
-- Применено к pinhead-os-v2 (glhwbktsokphgksdvcxj) 2026-07-17 через MCP apply_migration.
...
begin
  ...
  loop
    ...
    -- Нанесения с параметрами (страницы «Нанесение №N» ТЗ)
    for v_print in
    ...
    -- Этапы маршрута: depends_on приходит индексами ранее вставленных этапов
    v_stage_ids := '{}';
    for v_stage in
