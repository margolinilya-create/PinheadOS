-- Подрядные этапы приезжают ВМЕСТЕ с заказом (правки заказчика 16.08, блок 2).
--
-- ЗАЧЕМ. Конструктор маршрута появился и в форме создания заказа: менеджер
-- правит предложенный маршрут ДО того, как заказ существует, и подряд —
-- часть этой правки. Секция `stages` payload-а принимала только цех, порядок
-- и зависимости, поэтому исполнитель терялся: заказ создавался, а подрядные
-- этапы становились нашими — молча, без единой ошибки.
--
-- Функция пересоздана ПОДЛИННЫМ текстом миграции 20260816120000; изменены
-- ровно две вещи — вставка этапа читает `executor`/`contractor`/`operation`,
-- и у подрядного этапа тем же оператором заводится карточка подрядчика.
-- Писать по памяти нельзя: сверка в этом проекте уже показывала расхождение
-- в пяти правилах из десяти.
--
-- Карточку заводит ТОТ ЖЕ приём, что в `erp_route_apply`. Второго писателя
-- связи `erp_subcontracting.stage_id` быть не должно — именно её отсутствие
-- (на боевой базе обе строки с `null`) и делало раздел «Подряд» тупиком.
