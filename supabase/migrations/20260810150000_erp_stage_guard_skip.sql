-- Пропуск этапа получает собственное правило в страже (правки заказчика 10.08).
--
-- Заказчик просил аварийный обход блокирующих механик. Для гейтов (материалы, ТЗ,
-- отгрузка) это отдельный режим в админке, а «этап ждёт предыдущий» — не проверка,
-- а сам маршрут: глобально отключать его нельзя. Точечный ответ — пропустить один
-- этап, и статус `skipped` для этого в схеме есть с самого начала.
--
-- Разбирая страж под эту кнопку, нашли щель: переход в `skipped` не подпадал ни под
-- одну ветку `new.status` и проходил БЕЗ ПРОВЕРКИ ПРАВ. То есть любой участник ERP
-- с любым правом на этапы мог объявить чужое задание пройденным. Закрываем тем же
-- коммитом, что и добавляем кнопку.
--
-- Функция пересоздаётся ЦЕЛИКОМ из подлинного текста миграции 20260805120000
-- (правило проекта: реконструкция по памяти уже расходилась с оригиналом в пяти
-- правилах из десяти). Изменение ровно одно — новая ветка `elsif new.status = 'skipped'`.
...
comment on function public.erp_stage_guard() is
  'Разбирает изменение этапа по колонкам: приоритет, перенос, брак, статусы и результат — каждое под своим правом; плановые даты — под order.manage либо stage.take при взятии в работу; пропуск этапа — под order.manage; чужой цех — только с правом переноса.';
