-- Файлы привязываются к СТРОКЕ ЛИСТА ЗАКУПКИ (правки заказчика 16.08, п. 14).
--
-- Документ перечисляет, что прикладывают к закупке: «фотография материала,
-- ссылка / скрин позиции поставщика, техническое описание, цвет, пример,
-- отделочный материал». Всё это относится к КОНКРЕТНОЙ строке, а не к заказу
-- целиком: свалить их в общую кучу — значит отдать закупщику десяток картинок
-- без ответа на вопрос, к чему они.
--
-- Материалов на момент выбора файла ещё не существует — их создаёт та же
-- транзакция, что и заказ. Поэтому в payload приезжает `material_index`,
-- а функция раскладывает его в uuid по массиву вставленных строк. Тот же приём,
-- что `item_index` у позиций и `depends_on` у этапов маршрута.
--
-- Функция пересоздана ПОДЛИННЫМ текстом миграции 20260816170000; изменены
-- ровно три вещи: сбор `v_mat_ids`, объявления под него и `material_id`
-- во вставке вложения.
alter table public.erp_order_attachments
  add column if not exists material_id uuid
    references public.erp_materials(id) on delete cascade;

create index if not exists erp_order_attachments_material_idx
  on public.erp_order_attachments (material_id) where material_id is not null;

comment on column public.erp_order_attachments.material_id is
  'Строка листа закупки, к которой относится файл. NULL — файл заказа или позиции.';
