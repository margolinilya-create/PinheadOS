-- Файлы подрядного этапа доезжают ИЗ ФОРМЫ СОЗДАНИЯ заказа (документ 20.08).
--
-- Девятое поле подрядного шага — «ТЗ / файлы». Схема под них заведена
-- (`erp_order_attachments.stage_id`, вид `subcontract`), карточка операции
-- умеет прикладывать их к существующему этапу. Не хватало последнего: этапа
-- в момент сборки маршрута ЕЩЁ НЕ СУЩЕСТВУЕТ, и файл, выбранный в форме,
-- привязать было не к чему.
--
-- РЕШАЕТСЯ ТЕМ ЖЕ ПРИЁМОМ, ЧТО СТРОКИ ЛИСТА ЗАКУПКИ: в payload едет
-- `stage_index` — номер этапа внутри своей позиции, — а функция превращает
-- его в `stage_id` уже созданного этапа. Второй, неатомарной привязки после
-- создания заказа не появляется: файл, показанный приложенным, обязан быть
-- привязан той же транзакцией, что и сам заказ.
--
-- КЛЮЧ СОСТАВНОЙ («позиция:этап»), а не сквозной номер: `v_stage_ids`
-- обнуляется на каждой позиции, и сквозной индекс сдвигался бы у всех
-- позиций ниже первой — ровно та ошибка, которую уже ловили на `material_index`.
--
-- Текст функции ПОДЛИННЫЙ, из действующего определения (в репозитории его нет:
-- 18.08 к проду применили миграции мимо репозитория, и связь заказа с ТЗ
-- `tz_order_id`/`tz_number` живёт только здесь). Изменены три места:
-- объявления счётчиков, накопление ключей в цикле этапов и вставка вложений.

-- Заголовок в нижнем регистре и тело в `$$` — как во всех миграциях проекта.
-- `pg_get_functiondef` отдаёт ЗАГЛАВНЫЙ вариант и `$function$`, а сторожевые
-- тесты ищут `create or replace function public.<имя>(` и разбирают тело между
-- `$$`: скопированный дословно вывод молча уводил их на предыдущую миграцию —
-- то есть сторож проверял бы не ту функцию, которая работает.
...
begin
  ...
  loop
    ...
    loop
      ...
      -- правилом, что в `erp_route_apply`.
      ...
      -- Ключ «позиция:этап» — по нему вложение находит свой этап
  ...
  loop
    -- Этап ищется по паре «позиция:этап». Отсутствие ключа читается как
    -- «файл не этапа», а не как ошибка: секцию шлют и старые сборки клиента
    v_at := case
    ...
    insert into erp_order_attachments
      (order_id, item_id, material_id, stage_id, file_path, file_name, kind, uploaded_by)
    values
      (v_order_id,
       case
