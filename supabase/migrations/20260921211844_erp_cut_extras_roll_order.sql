-- Плюс закроя раскладывается по НОМЕРУ РУЛОНА, а не по случайному uuid
-- (правка заказчика 21.09, п. 3; исправление к 20260921211524).
--
-- ЧТО БЫЛО НЕ ТАК. Внутри ячейки «цвет × размер» плюсом объявляются последние
-- штуки — те, что перешагнули план. Строки при этом упорядочивались по `z.id`,
-- а это `gen_random_uuid()`: «последней» оказывалась не та строка, которая
-- перешагнула план, а та, чей идентификатор случайно больше. Проверено
-- на живом заказе «тест закрой 1» (план XS 200): рулон №1 сдал 30, рулон №2 —
-- 175, и плюс 5 лёг на рулон №1, который до плана не дотянул в одиночку.
--
-- Сумма при этом была верной, и ни один читатель per-row `qty_extra` сегодня
-- не смотрит — но «цифра стоит не у того рулона» это ровно тот разряд ошибок,
-- который замечают в цеху и не могут объяснить. Порядок берётся у сквозного
-- номера рулона (`erp_material_rolls.seq`): в цеху рулоны и называют номерами.
-- Строки без рулона (обычная размерная сдача) идут после, по `id`.
...
begin
  ...
  if v_rolled then
     ...
     * план: раскладка идёт накопительным итогом ПО НОМЕРУ РУЛОНА, то есть
     * плюс достаётся тому рулону, на котором план кончился. Сумма плюсов
     * по строкам равна плюсу ячейки — «отдельно по каждому размеру
