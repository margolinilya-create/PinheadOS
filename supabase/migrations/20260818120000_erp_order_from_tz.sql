-- Связь «ТЗ → производственный заказ» (мост из Order Studio).
--
-- ЗАЧЕМ. Менеджер оформлял техническое задание в визарде, а потом ПЕРЕБИВАЛ
-- тот же заказ руками в форму производства: размерную сетку, нанесения,
-- упаковку, техблок. Расхождение ТЗ с производственным заказом обнаруживалось
-- в цехе. Теперь у утверждённого ТЗ есть кнопка «В производство», и заказ
-- заводится из его данных.
--
-- ПОЧЕМУ УНИКАЛЬНЫЙ ИНДЕКС, А НЕ ПРОВЕРКА В КНОПКЕ. Создание заказа и запись
-- обратной ссылки в `orders` — два запроса, и второй может не пройти: RLS
-- на UPDATE запрещает через `USING`, то есть отдаёт «0 строк», а не ошибку.
-- Повторное нажатие после такого сбоя завело бы ВТОРОЙ производственный заказ
-- по тому же ТЗ — с этапами, закупкой и очередью в цехах. Индекс делает это
-- невозможным на сервере: повтор падает с 23505, и клиент показывает
-- «уже отправлен» со ссылкой на существующий заказ.
--
-- ПОЧЕМУ РЯДОМ ХРАНИТСЯ НОМЕР ТЕКСТОМ. Таблицу `orders` цех не видит вовсе:
-- политика `orders_select` открывает её только admin/director/rop и
-- менеджеру-автору. Ссылка по `tz_order_id` у рабочего отдала бы пусто —
-- а номер ТЗ рядом с заказом читается всеми и без второго запроса.
--
-- `on delete set null`, а не каскад: удалённое ТЗ не должно уносить
-- производственный заказ, по которому цех уже работает.
...
-- Обратная ссылка: по ней Order Studio показывает «уже в производстве» вместо
-- кнопки. Отдельная колонка, а не запрос в `erp_orders`, потому что список ТЗ
-- рисуется одним запросом и подтягивать к нему производственные заказы
-- значило бы делать второй — на каждое открытие доски.
--
-- Источником правды остаётся `erp_orders.tz_order_id` с его уникальным
-- индексом: эта колонка — подсказка интерфейсу, и её незаписанность (сбой
-- между двумя запросами) не создаёт второго заказа, а лишь показывает кнопку
-- ещё раз. Повторное нажатие упрётся в 23505 и объяснит, что заказ уже есть.
...
-- Функция пересоздана ПОДЛИННЫМ текстом миграции 20260816210000 (правило
-- проекта: не по памяти). Изменены ровно две строки — список колонок заказа
-- и список значений. Колонка, добавленная в `erp_orders`, попадает сюда тем же
-- коммитом: иначе форма её показывает, менеджер заполняет, а в базу не
-- приезжает ничего.
