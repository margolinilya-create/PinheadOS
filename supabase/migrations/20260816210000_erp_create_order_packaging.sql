-- Поля упаковки изделия сохраняются при создании заказа (правки 16.08, п. 1).
--
-- Миграция 20260816200000 завела `packaging_size`, `sticker_place` и
-- `marking_place`, но `erp_create_order` перечисляет колонки позиции ПОИМЁННО —
-- значит, новые поля молча терялись бы при создании заказа: форма их
-- показывает, менеджер заполняет, в базу не приезжает ничего. Колонка,
-- добавленная в `erp_order_items`, попадает в эту функцию тем же коммитом.
--
-- Функция пересоздана ПОДЛИННЫМ текстом миграции 20260816190000; изменены
-- ровно две строки — список колонок и список значений.
