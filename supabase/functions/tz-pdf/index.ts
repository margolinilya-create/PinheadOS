 ...
 *
 * ЧТО БЫЛО. Гейт ТЗ (`utils/tz.stageMissingTz`) держит производственные цеха,
 * пока к позиции не приложен документ, — а документ делал человек вручную
 * в другой программе и загружал файлом. Это был единственный шаг заказа,
 * который система не умела делать сама, и одновременно тот, без которого
 * не начинает работать ни один цех.
 *
 * Формат — `docs/erp/tz-format-analysis.md`: шапка заказа, изделие с техблоком,
 * размерная сетка, нанесения с параметрами, бирки, упаковка. Всё это уже есть
 * в модели (правки 16.08), поэтому документ собирается из полей заказа.
 *
 * ДОКУМЕНТ ПРИНАДЛЕЖИТ ПОЗИЦИИ. `erp_tz_documents.item_id` — позиция; так его
 * видят все цеха её маршрута (`itemTzDocument`), и назначать документ каждому
 * цеху не нужно. На заказ из трёх изделий выходит три документа: у каждого
 * своя размерная сетка и свои нанесения, и общий файл цех читал бы, отыскивая
 * в нём свою позицию.
 *
 * ПОЧЕМУ ИМЕННО `erp_tz_documents`, А НЕ ВЛОЖЕНИЕ. Лист закупки лежит
 * вложением вида `purchase` СПЕЦИАЛЬНО, чтобы гейт ТЗ не счёл его техническим
 * заданием и не открыл цехам работу без ТЗ. Здесь наоборот: документ обязан
 * попасть в таблицу, которую гейт считает, — иначе он не снимет блокировку.
 *
 * ПОВТОРНАЯ СБОРКА — НОВАЯ ВЕРСИЯ, а не перезапись: правка ТЗ после запуска
 * обязана быть видна цеху бейджем «ТЗ обновлено» (`tz.tzUpdatedAfterStart`),
 * а перезаписанный файл сменился бы у всех молча, и человек, работающий
 * по распечатке, не узнал бы об изменении. Версии живут в одной `group_id`;
 * `is_current` снимается ПЕРЕД вставкой новой — обратный порядок падает
 * с 23505 на частичном уникальном индексе.
 */
...
/** Размерная сетка позиции: строки «цвет → размеры» */
...
Deno.serve(async (req: Request) => {
  ...
  /**
   * Личность — из ТОКЕНА, права — той же функцией, что стоит в политиках
   * таблиц заказа. Своя проверка роли разошлась бы с политикой в первую же
   * её правку; в этом проекте на таком расхождении уже ловились.
   */
  ...
  // Данные читаем ОТ ЛИЦА ВЫЗЫВАЮЩЕГО: право видеть заказ проверяет RLS,
  // и обходить её сервисным ключом значит собрать ТЗ по чужому заказу
  ...
  for (const item of rows) {
    ...
    try {
      ...
      /** Новая страница, когда до низа осталось меньше нужного запаса */
      ...
      if (tech.length > 0) {
        ...
        for (const [label, value] of tech) {
          // Многострочные поля (бирки приезжают из ТЗ несколькими строками)
          // печатаются построчно: pdf-lib сам переносы не делает и рисует
          // «\n» как пробел, склеивая всё в одну нечитаемую строку
    ...
    // ── Версия документа ─────────────────────────────────────────────────────
    /**
     * Ищем ПРЕЖНИЙ системный документ этой позиции, чтобы продолжить его
     * группу версий. Признак — путь: `tz/auto/<order>/<item>/…`. По имени
     * файла искать нельзя: человек мог загрузить свой PDF с тем же именем,
     * и мы бы записали новую версию поверх ЧУЖОГО документа.
     */
    ...
    /**
     * Снимаем `is_current` со старой версии ПЕРЕД вставкой новой — правило
     * проекта: обратный порядок падает с 23505 на частичном уникальном
     * индексе `unique (group_id) where is_current`.
     */
    if (previous) {
    ...
    if (rowErr) {
      ...
      // Прежняя версия при этом остаётся без `is_current`, поэтому её флаг
      // возвращаем: иначе у позиции не окажется актуального ТЗ вовсе,
      // и гейт остановит цех из-за нашей ошибки.
