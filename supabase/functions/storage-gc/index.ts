...
/**
 * Таблицы и колонки, ДЕРЖАЩИЕ ключ объекта в этом бакете.
 *
 * Список ОТСТАВАЛ ОТ СХЕМЫ: проверкой 13.09 носителей было два, а 15.09
 * появился третий — `erp_sku_card_files.file_path`, снимок ключа вложения
 * разработки (карточка модели файлы не копирует, она на них ссылается).
 * Девять дней уборка считала техпакет модели ничьим. Поэтому состав теперь
 * сторожит тест `erp/utils/storageGc.test.ts`: он читает миграции и требует,
 * чтобы каждая таблица с колонкой `file_path` стояла здесь. Новый носитель
 * без записи в этом списке — красный CI, а не стёртый файл.
 */
const REFERENCES = [
