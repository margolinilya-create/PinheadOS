 ...
 *
 * ПОТОЛОК ОБОЛОЧКИ СНИЖЕН 10.08 с 250 до 222 кБ. Он подошёл к пределу
 * (249 из 250) и перестал бы что-либо сторожить: любая правка ломала бы сборку,
 * а не ловила регрессию. Разбор показал две причины веса — карточка заказа
 * ехала в оболочке из-за хоста, смонтированного вне `<Routes>`, и три «первых»
 * экрана были статикой ради отсутствия мигания. И то и другое стало ленивым,
 * оболочка упала до 210 кБ. Новый потолок оставляет ~12 кБ на обычный рост
 * и ловит возврат экрана в критический путь: один такой возврат — это 30 кБ.
 */
const BUDGETS = {
  shellJs: 222_000,
  shellCss: 40_000,
