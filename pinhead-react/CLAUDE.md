...
## Правила входа и сессии (сессия 33)

- Обёртка `fetch` для Supabase — `lib/authFetch.withRefreshTimeout`: потолок 10 с
  на `grant_type=refresh_token` и больше ни на что. Обновление токена работает
  внутри общего лока сессии и своего таймаута не имеет
- Перехваченный лок (`utils/i18n.isAuthLockFailure`) — повод повторить: запрос
  не ушёл. Повторяют `useAuthStore.fetchProfile` (один раз) и `erpRead` (наравне
  с обрывом связи). Текст для человека один — `AUTH_LOCK_MESSAGE`
- `watchAuthState` не ходит в базу из обработчика события: вызов откладывается
  на следующий тик, иначе он встаёт в очередь удерживаемого лока
- Загрузка профиля дедуплицируется по id (`profileInFlight`) — `SIGNED_IN`
  приходит по три подряд, и гейт «профиля в памяти нет» их не разводит
- Загрузка файлов — только `store/shared.uploadResilient`: при сбое без ответа
  сервера он спрашивает бакет (файл мог дойти), считает «уже существует» успехом
  и делает один повтор. Отказ прав и `InvalidKey` не повторяются
- Ключ вложения заказа даёт `attachmentPath` (метка времени + случайный хвост):
  `uploadResilient` перезаписывает свой ключ, и уникальность обязательна
