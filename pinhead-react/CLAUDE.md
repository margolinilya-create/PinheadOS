...
## Правила сессии 51 (понятная логика, волна 1): где что лежит

- Значения аудита переводит ТАБЛИЦА `AUDIT_VALUE_LABELS`
  (`orderCard/HistorySection.jsx`), а не цепочка `if`. До 05.09 переводились
  пять полей, а `material.*`, `subcontract.*` и `procurement.status`
  показывались сырыми: менеджер читал «ordered → in_transit» и «sent →
  received_at_pinhead» — 150 записей из 672 на проде. Подписи всё это время
  лежали в `types.ts`: не хватало не словаря, а его применения
- У `subcontract.status` СЛИТЫЙ словарь (`SUBCONTRACT_STATUS_LABELS` +
  `SUBCONTRACT_PHASE_LABELS`) — это не ошибка: триггер переведён на `phase`
  миграцией `20260905071500`, но 37 записей, сделанных раньше, лежат в старом
  словаре. История задним числом не переписывается
- Журнал видит то, ради чего заведён: у материала добавлены `qty_ordered`,
  `ordered_on`, `price`, у подряда — `qty_sent`, `qty_returned`, `qty_defect`.
  Раньше «сколько заказали и почём» и «сколько отдали подрядчику» следа
  не оставляли вовсе
- Сторож — `orderCard/historyLabels.test.ts`: читает поля ИЗ САМИХ ТРИГГЕРОВ
  (`latestMatching` по `create trigger`), а не из списка в тесте — список
  разошёлся бы с базой в первую же правку. Отдельным утверждением проверяет,
  что триггер подряда следит за ЖИВОЙ колонкой: первая редакция сторожа была
