...
- Гейт ТЗ в форме принимает ВТОРОЙ способ выполнения — «соберёт система»:
  `validateTzDocs(items, docs, systemWillGenerate)`, флаг ставит `CreateOrderModal`
  при наличии `prefill.tzOrderId`. Без него мост не работал вовсе
- `useTzPrefill` подставляет каталогам запасные значения из `src/data`
  (`withDefaults`): в боевом `app_config` нет строки `zonesCatalog`
- `erp/utils/orderStage.ts` — «где заказ сейчас» одной фразой (колонка «Стадия»
  списка заказов, карточка планшета, колонка «Производство» списка ТЗ);
  `erp/utils/myOrders.ts` — чип «Мои» в списке заказов
- Правила и их причины — в корневом `CLAUDE.md`, разделы «Правила моста»
  и «Стадия заказа у менеджера»
...
**Флаги прогона e2e живут в `.env.e2e`** (`npm run dev -- --mode e2e`),
а не в переменных команды `webServer`: этот Vite наполняет `import.meta.env`
ТОЛЬКО из `.env`-файлов, переменные окружения процесса туда не попадают.
Прежняя строка `VITE_FEATURE_ORDER_STUDIO=1 VITE_DEV_AUTOLOGIN=1 npm run dev`
выглядела рабочей и не делала ничего: автологин не включался, приложение
показывало форму входа, и падал ВЕСЬ прогон — 187 сценариев с одинаковым
снимком. Проверяется одним запросом к живому серверу:
`curl localhost:5173/src/config/features.ts` печатает подставленный env.
В `.env` эти флаги класть НЕЛЬЗЯ — его читает и Vitest, а там пять тестов
проверяют ровно обратное («orderStudio выключен по умолчанию», «без автологина
показывается форма входа»). Диагноз тот же, что у пустого `.env` ниже:
сначала проверьте окружение, потом спеки.

**`reuseExistingServer: true` подхватывает ЧУЖОЙ dev-сервер на 5173.** Свой
сервер, поднятый для ручной проверки (например, QA-мост с
`QA_SB_BRIDGE=1`), Playwright примет за свой — со своим `.env`, без флагов
e2e. Симптом тот же: массовые падения на экране входа. Перед прогоном
проверяйте, что порт свободен.
