...
- **Тест может закреплять дефект.** За сессию нашлось четыре таких: формула
  округления срочности в `pricing.ts`, `not.toContain('size_grid')` в стороже
  и в тесте стора, `not.toContain('eq:status=active')` у архива. Прежде чем
  чинить — ищите тест, который описывает нынешнее поведение; если он есть,
  переписывать надо оба
- **Усилив сторожа, сломайте то, что он охраняет, и убедитесь, что он покраснел.**
  Сторож колонок этапа выглядел починенным и проверял 5 колонок из 22
- **Замер производительности — «было → стало» в ОДНОМ прогоне.** Прежняя
  реализация воспроизводится рядом в том же харнессе, иначе разброс от загрузки
  машины больше эффекта
- `utils/stagesById` — общий указатель `id → этап` с памяткой по массиву.
  Массивы этапов НЕ мутировать по месту: памятка отдаст устаревшую карту молча
- Откат мутации — `restoreOrderIn` (точечный, по одному заказу), НЕ
  `set({ orders: prev })`: снимок всего массива стирает соседнюю успешную мутацию
- Ручные типы лежат в ДВУХ файлах: `erp/types.ts` и `erp/store/types.ts`.
  Сторож `types/schema.test.ts` сверяет схему в ОБЕ стороны и читает оба
- Потолок одновременных тостов — 4 (`useToastStore`). Счётчика одинаковых мало:
  `erpError` подставляет имя действия, и при одном сбое тексты разные
- Список позиций формы заказа — со стабильным ключом (счётчик), не `key={i}`
...
## Тесты
```bash
npm run test      # 2400 unit тестов (Vitest)
