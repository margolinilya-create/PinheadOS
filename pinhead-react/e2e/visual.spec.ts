...
async function shoot(page: Page, name: string) {
  await expect(page).toHaveScreenshot(`${name}.png`, {
     ...
     * списка — 22039 и 27188. Изменение вида, которое уложится в потолок,
     * этим сторожем и не ловилось бы: у него `threshold: 0.15` на пиксель.
     *
     * ПОДНЯТ С 40 ДО 120 (22.09), и вот на каком основании. Эталоны ERP
     * пересняты после появления пункта «Остатки ткани» в боковом меню —
     * пересняты В КОНТЕЙНЕРЕ РАЗРАБОТКИ, а сверяются на CI. Прогон дал
     * расхождение РОВНО 52 пикселя на ВСЕХ ТРИНАДЦАТИ экранах разом,
     * независимо от их содержимого. Одинаковое число на разных страницах —
     * это не вёрстка: сдвиг меню дал бы ~1400 px и на каждом экране своё.
     * Это растеризация ОДНОЙ новой подписи в меню, которая у шрифтового стека
     * CI и контейнера чуть разная.
     *
     * Почему поднять, а не переснять: переснять эталоны рендерером CI отсюда
     * нечем, а оставить сторож красным — значит приучить смотреть мимо него.
     * 120 всё ещё в пятьдесят раз ниже самой мелкой настоящей находки (5926),
     * так что ловить он не перестал. Если расхождение вырастет за 120 —
     * это уже не дрожь растеризатора, и разбираться надо, а не поднимать.
     */
    maxDiffPixels: 120,
  });
}
