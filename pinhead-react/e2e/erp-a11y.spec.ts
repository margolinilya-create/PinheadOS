...
test.describe('Формы и объявления', () => {
  ...
  const FIELD_SCREENS = [
    ...
    /**
     * ЗАКУПКА ОТКРЫВАЕТСЯ НА КАРТОЧКЕ ЗАКАЗА. Поля закупки живут внутри неё
     * (`PurchaseCard`), а на голом `/purchasing` видимых полей нет вовсе —
     * до 13.09 проверка натыкалась там на единственный оставшийся `input`,
     * и это была строка ГЛОБАЛЬНОГО ПОИСКА из шапки, снятая правкой п. 1.
     * То есть на этом экране сторож проверял чужое поле, а не поля закупки.
     */
    ['Закупка', '/purchasing?studio=0&supply=ord-a'],
