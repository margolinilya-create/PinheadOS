...
test.describe('Рабочая очередь цеха (правки 2, 3, 9)', () => {
   ...
   */
  /**
   * ГРУППА ОЖИДАНИЯ РАСКРЫТА (правка заказчика 30.08, п. 7).
   *
   * РЕШЕНИЕ 23.08 ОТМЕНЕНО САМИМ ЗАКАЗЧИКОМ, и вот почему оно не сработало:
   * у закроя ожидание почти всегда снабженческое, то есть свёрнутой
   * оказывалась ЕДИНСТВЕННАЯ непустая группа. Экран честно показывал
   * «Готово к запуску: 0» и ничего больше — а документ описывает это как
   * «в Закрое заказ отсутствует до завершения закупки». Заказ там был,
   * просто свёрнутым внизу.
   */
  test('«Ожидают материалы» раскрыты при первом открытии', async ({ page }) => {
    ...
    // Свернуть по-прежнему можно — снято именно умолчание, а не возможность
    await toggle.click();
    await expect(toggle).toHaveAttribute('aria-expanded', 'false');
  });

  /**
   * ГРАНИЦА ДВУХ ОЖИДАНИЙ (пп. 2 и 3 документа 23.08) НЕ ТРОНУТА: незакрытая
   * ЗАКУПКА — снабжение, а не производство, и у закроя группа ожидания
   * по-прежнему одна. Изменилось только то, что она раскрыта: работа,
   * назначенная на цех, обязана быть видна сразу (п. 7 документа 30.08).
   */
  test('ожидание закупки — снабжение, и у закроя группа ожидания одна', async ({ page }) => {
    ...
    // Причина видна БЕЗ раскрытия — за этим правку и делали
  ...
  test('строка очереди показывает заказ, срок, статус и готовность', async ({ page }) => {
    ...
    /**
     * Подпись прогресса называет ТРИ числа с 12.09 (правка п. 5): «Заказ N ·
     * Факт M», и «Плюс +K» при перевыполнении. Прежде здесь было одно
     * «Сделано N из M шт» — факт сверх тиража в него не помещался вовсе.
     */
    await expect(row.getByTitle(/Заказ \d+ · Факт \d+/)).toBeVisible();
  });
...
test.describe('Админка: права и справочники (правки 11 и 12)', () => {
   ...
   *
   * Обход 04.09 добавил второе обязательное поле: ОБЛАСТЬ. Раньше в «Где»
   * умолчанием стояла «Вся система» — самый широкий из вариантов при подсказке
   * рядом «снимайте точечно», — и человек мог снять проверку по всей фабрике,
   * ни разу не подумав об области.
   */
  test('аварийный режим: вкладка открывается и требует причину и область', async ({ page }) => {
    ...
    // Причина есть, области нет — по-прежнему нельзя
    await expect(submit).toBeDisabled();

    await page.getByLabel(/Где/).selectOption({ label: 'Вся система (все заказы)' });
