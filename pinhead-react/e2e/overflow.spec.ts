...
for (const route of ROUTES) {
  test(`горизонтальная прокрутка под контролем: ${route}`, async ({ page }) => {
...
test('канбан из шести цехов сообщает о прокрутке', async ({ page }) => {
...
test('таблица «Заказы в работе» помещается целиком', async ({ page }) => {
  /*
