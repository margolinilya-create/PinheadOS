...
async function openDev(page: Page, name: string) {
  ...
  /**
   * ПЕРЕХОД ДОЖИДАЕТСЯ ПО НОВОМУ ЭКРАНУ, А НЕ ПО АДРЕСУ.
   *
   * Пока открытие было кликом по всей строке таблицы, следующий локатор
   * успевал сняться уже на карточке. Ссылка уводит роутером, и ждать адрес
   * НЕДОСТАТОЧНО: React Router ведёт навигацию через `startTransition`,
   * а страница разработки ленивая (`lazyScreen`) — пока её чанк грузится,
   * React держит на экране СТАРЫЙ UI при уже обновлённом `location`.
   * Доска в этот момент жива, и `getByRole('list', { name: 'Путь
   * разработки' })` находил пять степперов (четыре карточки доски плюс один
   * со страницы) — strict mode violation на верном коде.
   *
   * Поэтому ждём то, чего на доске нет вовсе, — постоянную справку страницы.
   */
  await expect(page).toHaveURL(/\/experimental\/[^/?]+/);
  await expect(page.getByRole('complementary', { name: 'Справка по разработке' }))
    .toBeVisible();
}
...
test.describe('Экран разработки: состояния считаются, а не хранятся', () => {
  ...
  /**
   * ПРАВКА ЗАКАЗЧИКА 20.09, П. 3: с карточки доски сняты ТЕКСТОВЫЕ ПОДПИСИ —
   * тип разработки, ответственный, текущая задача-блокер и «следующее
   * действие». Остались номер сделки, название изделия, срок и ЧИПЫ
   * состояния.
   *
   * Прежние две спеки требовали ровно снятый текст («Подбор материала»,
   * «снять блокировку: …»), то есть сторожили то, что документ просил
   * убрать. Переписаны на новое правило, и обе половины проверяются вместе:
   * текста на доске нет, а данные никуда не делись — они в карточке
   * разработки. Без второй половины «убрали» неотличимо от «потеряли».
   */
  test('на карточке доски нет текстовых подписей — только состояние', async ({ page }) => {
    await gotoDev(page, '/experimental?studio=0');
    const row = devRow(page, 'Бомбер двухслойный');

    // Номер и название остаются — по ним карточку и находят
    await expect(row).toContainText('Бомбер двухслойный');
    // А описательные строки сняты
    await expect(row).not.toContainText('снять блокировку');
    await expect(row).not.toContainText('Следующее действие');
  });

  test('снятое с доски доступно в карточке разработки', async ({ page }) => {
    await gotoDev(page, '/experimental?studio=0');
    await devRow(page, 'Бомбер двухслойный').getByRole('link').first().click();

    // Карточка отвечает на «что именно стоит» — тем текстом, который ушёл с доски
    await expect(page.getByText('Подбор материала').first()).toBeVisible();
  });
...
test.describe('Карточка разработки', () => {
  ...
  test('маршрут показан stepper-ом, у каждого этапа видно состояние', async ({ page }) => {
    ...
    for (const label of [
      'Построение лекал', 'Ожидает материалы', 'Крой', 'Нанесения', 'Пошив', 'Финальный этап',
    ]) {
 ...
 *
 * Шестая колонка — «Ожидает материалы» (правка 01.09, п. 1): «сейчас после
 * Построения лекал сразу идёт Крой. Нужно добавить между ними отдельный этап».
 */
test.describe('Доска экспериментального цеха', () => {
  test('шесть колонок документа, и «Нанесения» ОДНА', async ({ page }) => {
    await gotoDev(page, '/experimental?studio=0');
    /**
...
test.describe('Канбан ЭКС: колонку ставит человек (п. 4.2)', () => {
  ...
  /**
   * НАСТОЯЩИЙ HTML5-БРОСОК. `locator.dragTo` его не воспроизводит: Playwright
   * шлёт мышиные события, а браузер начинает жест перетаскивания сам и только
   * для настоящего пользовательского ввода. Поэтому события собираются вручную
   * с ОБЩИМ `DataTransfer` — тем самым, который обработчик `dragstart` заполняет
   * и который читает `drop`.
   *
   * Сторожу это нужно ровно потому, что запреты последовательности кнопками
   * не нарушить: «‹ ›» ходят по соседям и хода через шаг не предлагают вовсе.
   * Проверка кнопкой была бы зелёной и на снятом запрете.
   */
  const dragCard = async (
    page: import('@playwright/test').Page, name: string, toColumn: string,
  ) => {
    await page.evaluate(({ name: n, toColumn: t }) => {
      const card = [...document.querySelectorAll('[role="listitem"]')]
        .find((el) => el.textContent?.includes(n));
      const col = [...document.querySelectorAll('section')]
        .find((el) => el.querySelector('header')?.textContent?.includes(t));
      if (!card || !col) throw new Error(`не нашёл карточку «${n}» или колонку «${t}»`);
      const dataTransfer = new DataTransfer();
      const fire = (el: Element, type: string) => el.dispatchEvent(
   ...
   *
   * 30.08 карточка не выпускалась С ЛЕКАЛ вовсе, и готовые лекала стояли
   * в одной колонке с недоделанными. 01.09 ожиданию дана своя колонка:
   * «система должна разрешить перенести карточку только в „Ожидает
   * материалы". Сразу в Крой перенести нельзя».
   *
   * У «Ветровки на молнии» материалы стоят `received` без приёмки складом —
   * то есть ровно «пришло, но склад не подтвердил».
   */
  test('с «Лекал» при неприбывшем материале ведёт ТОЛЬКО в стоянку',
    async ({ page }) => {
      await gotoDev(page, '/experimental?studio=0');
      ...
      // Кнопка «›» предлагает ровно тот ход, который разрешён: предлагать
      // «Крой» и тут же его отклонять значило бы ловить человека на пустом
      await blocked.getByRole('button', { name: 'Перенести в «Ожидает материалы»' })
        .click();
      const dialog = page.getByRole('dialog', { name: /Завершить построение лекал/ });
      await dialog.getByLabel('Техническое название лекал').fill('PNHD-J01-v1');
      await dialog.getByRole('button', { name: /Завершить и перенести/ }).click();

      await expect(column(page, 'Ожидает материалы')).toContainText('Ветровка на молнии');
      await expect(column(page, 'Построение лекал')).not.toContainText('Ветровка на молнии');
    });

  test('со стоянки в «Крой» не пускает, пока материал не принят складом',
    async ({ page }) => {
      await gotoDev(page, '/experimental?studio=0');
      const blocked = card(page, 'Ветровка на молнии');
      await blocked.getByRole('button', { name: 'Перенести в «Ожидает материалы»' })
      ...
      await expect(page.getByRole('status', { name: 'Оповещения' }))
        .toContainText('Закупка ещё не завершена');
      await expect(column(page, 'Ожидает материалы')).toContainText('Ветровка на молнии');
      await expect(column(page, 'Крой')).not.toContainText('Ветровка на молнии');
    });

  /**
   * ПЕРЕСКОК ОБЯЗАТЕЛЬНОГО ШАГА (правка заказчика 01.09, п. 2): «свободное
   * ручное движение оставляем, но система должна блокировать попытку
   * перескочить обязательный этап».
   *
   * Проверяется БРОСКОМ, а не кнопкой: кнопки ходят по соседям и такого хода
   * не предлагают вовсе — то есть кнопкой этот запрет не нарушить. Единственный
   * способ его нарушить — перетаскивание, и сторож обязан идти тем же путём,
   * иначе он зелен на сломанном коде.
   */
  test('из «Кроя» мимо заказанных нанесений в «Пошив» не пускает',
    async ({ page }) => {
      await gotoDev(page, '/experimental?studio=0');
      ...
      // У позиции заведена шелкография — «Нанесения» обязательны
      await dragCard(page, 'Ветровка образец', 'Пошив');

      await expect(page.getByRole('status', { name: 'Оповещения' }))
        .toContainText('Нанесения');
      await expect(column(page, 'Крой')).toContainText('Ветровка образец');
