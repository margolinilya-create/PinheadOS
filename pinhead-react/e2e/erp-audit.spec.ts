...
test.describe('Список заказов: страницы, сортировка, контекст в адресе', () => {
  ...
  test('три быстрых клика подряд доводят сортировку до конца цикла', async ({ page }) => {
     ...
     * следующий шаг считался от значений РЕНДЕРА: между сменой адреса и
     * перерисовкой кнопка несла устаревший обработчик, и клик в это окно
     * вычислял шаг от старого направления — третий клик вместо «снять
     * сортировку» снова давал «по убыванию», то есть не делал ничего.
     *
     * Здесь клики идут БЕЗ ожидания между ними — так по заголовку и тыкают,
     * когда торопятся. Предыдущий тест ждёт перерисовку после каждого клика
     * и этой ветки не проверяет по построению.
     */
    await page.goto('/orders?studio=0');
    const byTitle = page.getByRole('button', { name: 'Заказ', exact: true });
    ...
    /**
     * Клики идут в ОДНОЙ задаче JS, а не через `locator.click()`: тот ждёт
     * готовности элемента, и на быстрой машине React успевает перерисоваться
     * между кликами — окно не воспроизводится, а сторож зеленеет впустую.
     * Три синхронных клика гарантируют, что все три обработчика взяты из
     * ОДНОГО рендера: ровно то состояние, в котором старая реализация
     * считала шаг от устаревшего направления.
     */
    await byTitle.evaluate((el: HTMLElement) => {
      el.click(); el.click(); el.click();
    });

    await expect(page).not.toHaveURL(/sort=/);
    await expect(page.getByRole('columnheader', { name: /Заказ/ }))
