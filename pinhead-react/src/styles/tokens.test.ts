...
/**
 * ШКАЛА Z-INDEX: ЛИТЕРАЛ, РАВНЫЙ СТУПЕНИ, — ЭТО ОБОЙДЁННАЯ СТУПЕНЬ.
 *
 * Уборка 13.09 записала `--z-header` и `--z-nav` в мёртвые токены: `var()`
 * на них не ссылался НИКТО. Вывод был неверен — их потребители написали то же
 * число литералом (`Header.module.css` — `z-index: 100` у шапки и `200`
 * у раскрытого меню). Токен не мёртв, он обойдён, и это ровно тот дефект,
 * от которого в проекте уже стоит сторож на отступы и кегли.
 *
 * Вторая половина дефекта опаснее: ЧЕТЫРЕ элемента сидели на `9999` —
 * тост, пропуск к содержимому, блокер навигации и окно горячих клавиш.
 * Равные значения означают, что порядок наложения решает порядок в DOM, —
 * ровно то, что комментарий у самой шкалы объявляет недопустимым.
 *
 * ПРОВЕРЯЕТСЯ ТОЧНОЕ РАВЕНСТВО СТУПЕНИ, а не «любой большой z-index».
 * Значения между ступенями (90 у полосы прогресса, 150 у шторки экспресс-
 * расчёта, 299/300 у мобильного меню карточки) остаются литералами
 * намеренно: у них нет своей ступени, и перевод был бы редизайном порядка
 * наложения, а не механической правкой. Мелкие 1–20 — локальный stacking внутри
 * компонента, к шкале приложения отношения не имеют.
 */
describe('шкала z-index', () => {
  const index = withoutComments(readFileSync(join(process.cwd(), 'src/index.css'), 'utf8'));
  const STEPS = new Map<string, string>(); // значение → имя токена
  for (const m of index.matchAll(/(--z-[\w-]+):\s*(\d+)\s*;/g)) {
    if (!STEPS.has(m[2])) STEPS.set(m[2], m[1]);
  }

  it('ступени объявлены и ни одна не повторяет значение другой', () => {
    const declared = [...index.matchAll(/(--z-[\w-]+):\s*(\d+)\s*;/g)].map((m) => [m[1], m[2]]);
    expect(declared.length).toBeGreaterThanOrEqual(5);
    // Дубль значения сделал бы карту «значение → токен» неоднозначной,
    // а саму шкалу — не шкалой: две ступени на одной высоте не упорядочены
    expect(declared.length, `дубли: ${declared.map((d) => d.join('=')).join(' ')}`)
      .toBe(STEPS.size);
  });

  it('ни один z-index не повторяет значение объявленной ступени литералом', () => {
    const bad: string[] = [];
    for (const { file, css } of SOURCES) {
      css.split('\n').forEach((line, i) => {
        if (/^\s*--/.test(line)) return; // объявление самой ступени
        for (const m of line.matchAll(/z-index:\s*(\d+)/g)) {
          const tok = STEPS.get(m[1]);
          if (tok) bad.push(`${file}:${i + 1} — z-index: ${m[1]} → var(${tok})`);
        }
      });
    }
    expect(bad, bad.join('\n')).toEqual([]);
