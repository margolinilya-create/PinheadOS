...
export interface ErpOrder {
  ...
  is_demo?: boolean;
  /**
   * Техническое задание, из которого создан заказ (мост из Order Studio).
   * NULL — заказ заведён формой напрямую (обычный путь для срочных и простых).
   *
   * Уникальный индекс в базе не даёт создать по одному ТЗ два заказа: запись
   * обратной ссылки в `orders` — отдельный запрос, и его сбой иначе привёл бы
   * к повтору с этапами, закупкой и очередью в цехах.
   */
  tz_order_id?: string | null;
  /**
   * Номер ТЗ текстом. Дублирование намеренное: таблицу `orders` цех не видит
   * по RLS вовсе, а номер ему нужен — иначе на месте ссылки пустота.
   */
