...
/**
 * `done` — «сдан, срок не задавали», и это НЕ синоним `done_on_time`.
 *
 * До 02.09.2026 значения не было, а `erp_ship_order` склеивал две ветки
 * через `or`: «сдали ровно в срок» и «срока не было вовсе» давали одинаковый
 * `done_on_time`. На проде так были помечены шесть из десяти закрытых
 * заказов — отчёт «сдано в срок» больше чем наполовину состоял из заказов,
 * у которых сравнивать было не с чем. То же правило, что у `percentOf`:
 * ноль в знаменателе — это «неизвестно», а не «готово».
 *
 * Фильтры проекта различают заказы по `status === 'active'` /
 * `status !== 'active'` (`orderHelpers`, архивная выборка `ordersSlice`),
 * а не перечислением архивных значений, поэтому новое значение попадает
 * в архив и мимо активных счётчиков само.
 */
...
export const ORDER_STATUS_LABELS: Record<ErpOrderStatus, string> = {
  active: 'В работе',
  // Без уточнения «вовремя»: сравнивать было не с чем, и подпись обязана
  // это признавать, а не выбирать сторону
  done: 'Сдан',
...
/**
 * Подпись статуса заказа — с фолбэком на само значение.
 *
 * ЗАЧЕМ ФОЛБЭК, ЕСЛИ ТАБЛИЦА `Record<ErpOrderStatus, string>` И ТАЙПЧЕК
 * ЛОВИТ ПРОПУСК. Тайпчек ловит его в СБОРКЕ, а дыра открывается в ВЫКЛАДКЕ:
 * миграция добавляет значение в базу раньше, чем на прод уезжает бандл,
 * который про него знает. Ровно это случилось со статусом `done` 02.09.2026 —
 * шесть заказов получили его на проде, а работавший бандл читал `undefined`
 * и рисовал пустую ячейку статуса в четырёх местах: строка списка, карточка
 * планшета, чип карточки заказа и значение сортировки.
 *
 * Пустая ячейка — это то самое «ничего не роняет, просто молча врёт», от чего
 * в проекте написано правило заводить новое значение перечисления во всех
 * местах разом. Правило верное, но оно про КОД; между кодом и базой остаётся
 * окно, и закрывает его только фолбэк.
 *
 * Возвращается сырое значение, а не «—» и не «Неизвестно»: сырой `done_late`
 * человек прочитает и поймёт, а прочерк неотличим от «статуса нет».
 */
export function orderStatusLabel(status: string | null | undefined): string {
  if (!status) return '';
  return ORDER_STATUS_LABELS[status as ErpOrderStatus] ?? status;
}
