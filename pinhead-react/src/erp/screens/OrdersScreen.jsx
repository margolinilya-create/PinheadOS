      ...
      {pageRows.length > 0 && !isCompact && (
        <ScrollHintBox className={styles.tableWrap} label="Список заказов">
          <table className={`${styles.table} ${styles.ordersTable}`}>
            {/*
              ШИРИНЫ КОЛОНОК ЗАДАНЫ ЯВНО (06.09).

              Браузер раздаёт их по содержимому ВМЕСТЕ С ЗАГОЛОВКОМ, а заголовки
              здесь набраны заглавными вразрядку. Из-за этого «№ СДЕЛКИ»
              забирала 115px под пятизначное число, «МЕНЕДЖЕР» — 118 под «Анна»,
              «СОЗДАН» — 98 под дату, одинаковую у всех строк, а «ЗАКАЗ»,
              единственная колонка с длинным текстом, получала 85 и переносилась
              ПОСРЕДИ СЛОВА: «Шоппер / ы эко…». То есть ширину получал тот, кому
              она не нужна, за счёт того, по чему строку и опознают.

              Числа выведены из САМИХ ЗАГОЛОВКОВ, а не подобраны на глаз: они
              набраны заглавными вразрядку и вместе с кареткой сортировки задают
              нижнюю границу колонки. Первая редакция этой правки дала «№ СДЕЛ|»
              и обрезанного «МЕНЕДЖЕР» — то есть ровно ту же ошибку, только
              зеркальную: сперва ширину отняли у названия, потом у подписей.

              Сумма фиксированных — 792 при 988 доступных, остаток целиком уходит
              названию (~196 вместо 85). `min-width` таблицы держит эти ширины
              на узком десктопе: ниже него включается горизонтальная прокрутка
              (её и подсказывает ScrollHintBox), а не сжатие колонок обратно
