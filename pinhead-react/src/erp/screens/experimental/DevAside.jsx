    ...
    <aside className={styles.devAside} aria-label="Справка по разработке">
      <div className={styles.devAsideCard}>
        ...
        */}
        ...
        {/*
          КОММЕНТАРИЙ ПО ПРОРАБОТКЕ (правка 13.09, п. 10) — результат этапа
          проработки, записанный при переносе карточки в «Нанесения».
          Показывается, только когда он есть: у разработки, которая до
          нанесений ещё не дошла, прочерк обещал бы поле, которого никто
          не заполнял. Правится тем же переносом — здесь чтение, как и всё
          в этой колонке.
        */}
        {dev.branding_note && (
          <Row label="Комментарий по проработке">{dev.branding_note}</Row>
        )}
