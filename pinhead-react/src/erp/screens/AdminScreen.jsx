 ...
 * `adminOnly` — вкладка только для профиля администратора.
 */
const TABS = [
  /*
   * «Пользователи» — под профилем администратора, а не под правом матрицы.
   *
   * Это зеркало серверной `is_admin()`, на которой стоят политики записи
   * в `erp_employees` и `profiles`. Гейта у вкладки не было вовсе: директор
   * заходил, менял роль и цех сотрудника, видел «сохранено» — и значение
   * возвращалось при перезагрузке, потому что RLS запрещает UPDATE через
   * `USING`, то есть «0 строк», а не ошибку.
   *
   * Чинить это расширением серверной политики не стали: состав прав —
   * решение заказчика, а не следствие дефекта. Отдельного права матрицы
   * под управление учётными записями сегодня нет, и заводить его здесь
   * значило бы принять такое решение мимоходом.
   */
...
export default function AdminScreen() {
  ...
  const tabs = TABS.filter((t) => (!t.needs || access.can(t.needs))
    && (!t.adminOnly || access.isAdmin));
  // Запасная вкладка — ПЕРВАЯ ДОСТУПНАЯ, а не жёстко «Пользователи»: у того,
  // кому она закрыта, жёсткое значение оставляло пустую панель под выбранной
  // вкладкой, которой нет в списке
