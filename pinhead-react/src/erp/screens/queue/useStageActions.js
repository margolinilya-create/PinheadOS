...
export function useStageActions() {
  ...
  const onDefect = useCallback(async (entry, opts, photo) => {
    ...
    /**
     * ПОДТВЕРЖДЕНИЕ РЕЗУЛЬТАТА (обход 04.09). Успешный возврат брака не давал
     * НИКАКОГО отклика: `reportDefect` говорит только об ошибках, панель
     * просто закрывалась, а на карточке менялся лишь невидимый `qty_rework`.
     * Рабочий не знал, записалось ли, — и у самого неприятного действия смены
     * это худшее из возможных молчаний. Текст называет ЧИСЛО и КУДА ушло:
     * возврат на предыдущий этап переоткрывает и промежуточные, и человек
     * должен видеть, что произошло именно то, что он подтвердил.
     */
    if (ok) {
      /**
       * Имя целевого участка резолвится ЗДЕСЬ, а не кладётся в payload:
       * состав payload — контракт мастера брака со слайсом (его сторожит
       * `StageActionsPanel.test.jsx`), и подмешивать в него поле ради подписи
       * значит расширять контракт ради текста.
       */
      const targetStage = opts.target && opts.target !== 'current'
        ? (entry.item.stages ?? []).find((s2) => s2.id === opts.target)
        : null;
      // Та же карта коротких имён, которой подписан диалог подтверждения:
      // тост обязан назвать то же, что человек только что подтвердил
      const targetName = targetStage ? deptNameById.get(targetStage.department_id) : null;
      toast.success(targetName
        ? `Брак записан: ${opts.qty} шт · возвращено в «${targetName}»`
        : `Брак записан: ${opts.qty} шт · переделка на этом этапе`);
    }
    if (photo && !photoOk) toast.warning('Брак записан, но фото не загрузилось');
    return ok;
  }, [reportDefect, uploadOrderAttachment, deptNameById]);
