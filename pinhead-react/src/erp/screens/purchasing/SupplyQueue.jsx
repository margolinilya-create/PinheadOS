...
export function SupplyQueue({ orders, supplyDept, onTake, onClose, onAddMaterial }) {
  ...
  return (
    <section style={{ marginBottom: 20 }}>
      ...
      {orders.length === 0 ? (
        /*
          H-11 отчёта QA 13.08.2026: прежний текст «Заказов, ожидающих закупки,
          нет» стоял прямо над таблицей, полной закупочных строк, и читался как
          противоречие. Противоречия не было — это разные вещи: здесь очередь
          УЧАСТКА (заказы с открытым этапом «Закупка»), ниже реестр всех
          закупочных позиций, включая закрытые. Пустое состояние обязано
          называть, что именно пусто, и что тогда за список ниже.
        */
        <div className={styles.emptyState}>
          Ни у одного заказа нет открытого этапа «Закупка» — очередь участка пуста.
          Таблица ниже — реестр закупочных позиций, включая уже закрытые.
        </div>
      ) : (
