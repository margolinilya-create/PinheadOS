...
export function PlanTaskCard({
  ...
  return (
    ...
    >
      ...
      {/*
        ФАКТ ЭТАПА РЯДОМ С ФАКТОМ ДНЯ (Б3 обхода 04.09, половина, доступная
        интерфейсу).
        Один результат вводится в ДВУХ местах и это разные числа: очередь цеха
        пишет ПРИРАЩЕНИЕ `erp_item_stages.qty_done`, план — АБСОЛЮТ за день
        в `erp_calendar_slots.qty_done`. Связки между ними нет ни триггером,
        ни расчётом, поэтому цех, отчитавшийся в очереди, в плане остаётся
        «факт 0», и руководитель видит невыполнение, которого нет.
        Выбор модели — за владельцем (см. §10.2 отчёта): либо факт дня
        выводится из приращений этапа, либо величины остаются двумя.
        Но пока их две, ОБЕ обязаны быть видны на обеих поверхностях —
        иначе расхождение молчит. Здесь показывается вторая: сколько
        по этапу сдано всего.
      */}
      {stage && item?.qty > 0 && (
        <div className={styles.subText}>
          по этапу всего: {stage.qty_done ?? 0} из {item.qty}
        </div>
      )}
