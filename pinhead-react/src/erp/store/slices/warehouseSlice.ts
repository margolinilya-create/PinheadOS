...
export const warehouseSlice: StateCreator<ErpStore, [], [], WarehouseSlice> = (set, get) => ({
  acceptMaterial: async (
    ...
    /*
     * ПРИЁМКА ЗАПИСЫВАЕТ ПРИХОД В ЖУРНАЛ. Без этого количество не попадало
     * НИКУДА.
     *
     * Волна 3.3 верно убрала прямую запись `qty_received` из карточки: колонку
     * ведёт триггер по журналу `erp_material_receipts`, и два писателя одной
     * колонки затирали бы друг друга. Но журнал так и остался ОТДЕЛЬНЫМ,
     * необязательным действием, которого ни один путь приёмки не выполняет:
     * пришедшее число уходило только в историю склада (`logWarehouseOp`),
     * а колонка оставалась пустой.
     *
     * На боевой базе это выглядело так: два материала со статусом «принято
     * полностью», план 100 и 50, `qty_received` — NULL, строк журнала — ноль.
     * То есть склад отчитался о полной приёмке, а сколько пришло, не знает
     * никто. Хуже того, защита от заниженной приёмки в карточке сравнивает
     * план с фактом и при пустом факте молчит — то есть не срабатывает ровно
     * в том случае, ради которого заведена.
     *
     * Пишем приход ДО решения о приёмке: приход — физический факт, приёмка —
     * решение по нему. Если запись решения не пройдёт, в базе останется
     * правдивое «материал пришёл» без отметки о приёмке — состояние
     * восстановимое. Обратный порядок оставлял бы «принято» без количества,
     * то есть ровно тот дефект, который чиним.
     *
     * Дозаписываем только НЕДОСТАЮЩЕЕ: если кладовщик уже вносил приходы
     * формой журнала, `qty_received` их сумма, и повторная запись удвоила бы
     * количество. Экран склада передаёт сюда эту же сумму — там дельта равна
     * нулю и журнал не трогается; цеховая кнопка «Материал поступил» передаёт
     * план при пустом журнале, и приход записывается целиком, как и обещает
     * её текст подтверждения.
     */
    const reported = Number(qty_received);
    const alreadyLogged = Number(material?.qty_received ?? 0);
    const missing = Number.isFinite(reported) ? reported - alreadyLogged : 0;
    if (missing > 0) {
      /*
       * Неудача журнала НЕ отменяет приёмку.
       *
       * `addMaterialReceipt` — самостоятельное действие со своей проверкой
       * («отклонение нужно объяснить»), и она сильнее, чем нужно здесь:
       * приёмка со статусом «пересорт» без комментария падала бы целиком.
       * Это был бы новый жёсткий отказ там, где раньше всё проходило, —
       * и цена его выше, чем у самой болезни: без приёмки встаёт цех.
       *
       * Поэтому приход пишется НАСТОЙЧИВО, но не блокирующе: не получилось —
       * человек видит, что именно не записалось, и вносит приход формой
       * журнала. Молчания при этом нет ни в одном случае: `addMaterialReceipt`
       * уже показал свою причину, мы добавляем последствие.
       *
       * Комментарий для отклонения гейтится В ФОРМЕ приёмки, поэтому на
       * практике сюда приходит заполненным.
       */
      const logged = await get().addMaterialReceipt(materialId, {
        qty: missing,
        acceptStatus: accept_status,
        comment: accept_comment ?? undefined,
      });
      if (!logged) {
        toast.warning('Приёмка оформлена, но приход не попал в журнал — внесите его формой «Приход»');
      }
    }
    ...
    /**
     * Количество не пишется в КОЛОНКУ (волна 3.3) — оно ушло в журнал выше.
     *
