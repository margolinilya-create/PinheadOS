...
/**
 * ТОЧЕЧНЫЙ ОТКАТ: вернуть один заказ к состоянию из снимка, не трогая соседей.
 *
 * Все откаты в сторе были написаны как `set({ orders: prev })`, где `prev` —
 * снимок ВСЕГО массива, снятый до начала своей мутации. Пока действие одно,
 * это работает. Но действия идут внахлёст: цех жмёт «Взять в работу» по одному
 * заданию, диспетчер в это же время двигает другое. Если первое действие
 * стартовало раньше и упало позже, его откат возвращал массив к моменту СВОЕГО
 * старта — и стирал соседнюю мутацию, которая успела и пройти на сервере,
 * и лечь в стор.
 *
 * Замер это подтвердил: действие A по заказу o1 падает через 40 мс, действие B
 * по заказу o2 успевает за 5 мс; на экране оба задания в «ожидает», хотя o2
 * в базе `in_progress`. Человек видит, как только что взятое в работу задание
 * само откатилось, а ошибка показана про ДРУГОЕ задание. Лечится следующим
 * realtime-событием или F5, но окно рассинхронизации настоящее.
 *
 * Здесь восстанавливается ровно тот заказ, которого касалась мутация;
 * остальные остаются такими, какие они сейчас, — вместе с чужими успешными
 * изменениями. Идентичность нетронутых заказов сохраняется, как и в
 * `patchStageIn`, — это важно для ререндеров.
 */
export function restoreOrderIn(
  current: ErpOrderFull[],
  snapshot: ErpOrderFull[],
  orderId: string | null | undefined,
): ErpOrderFull[] {
  if (!orderId) return snapshot;      // заказ неизвестен — прежнее поведение
  const before = snapshot.find((o) => o.id === orderId);
  if (!before) return current;        // заказа не было в снимке — откатывать нечего
  return current.map((o) => (o.id === orderId ? before : o));
}
