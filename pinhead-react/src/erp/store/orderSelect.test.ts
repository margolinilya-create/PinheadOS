...
describe('списочный запрос заказов', () => {
  ...
  const ITEM_COLUMNS_NOT_ASKED: Record<string, string> = {
    ...
    /**
     * СТОИМОСТЬ СБОРКИ (правка 16.09, п. 6) списочным экранам не нужна:
     * её спрашивают форма сдачи швейки (там полный заказ) и будущая
     * аналитика (там серверная агрегация). Возить три колонки по всем
     * заказам списка ради экрана, который их не показывает, — тот же вес
     * без пользы, что у размерной сетки.
     */
    assembly_cost_per_unit: 'стоимость сборки читают карточка и аналитика, не список',
    assembly_cost_set_at: 'отметка времени стоимости: та же причина',
    assembly_cost_by: 'автор стоимости: та же причина',
  };
