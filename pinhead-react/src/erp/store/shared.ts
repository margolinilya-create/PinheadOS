...
/**
 * ОТВЕТ ЗАГРУЗЧИКА ЗАПОЗДАЛ: на старте раздел был пуст, а к моменту ответа
 * его наполнил ДРУГОЙ путь.
 *
 * Разделы `experimental` и `subcontracting` приезжают ДВУМЯ дорогами — пакетом
 * оболочки `erp_bootstrap` и своим `load*` у экрана, — и обе стартуют при
 * открытии раздела. Кто придёт вторым, тот и затрёт: экран уже отрисовался,
 * человек успел нажать кнопку, а запоздавший ответ ставит СНИМОК ДО ПРАВКИ.
 * Карточка молча возвращается назад, и повторное нажатие выглядит как «сайт
 * не работает».
 *
 * В e2e это проявляется как флака — «иногда не срабатывает», причём тем чаще,
 * чем медленнее машина: на CI с двумя воркерами падало, локально нет. Тот же
 * класс гонки, от которого у realtime стоит `pendingMutations`.
 *
 * СРАВНИВАЕТСЯ СОСТОЯНИЕ ДО И ПОСЛЕ ОЖИДАНИЯ, а не просто «загружено ли
 * сейчас»: иначе кнопка «Повторить» перестала бы обновлять раздел — при
 * повторе он загружен по определению. Пропускается ровно один случай:
 * на старте пусто, к ответу — наполнено кем-то другим.
 */
