...
describe('useErpStore — reportProgress (частичная готовность)', () => {
  ...
  /**
   * ПОТОЛОК СНЯТ (правка 12.09, п. 5). Прежде тест назывался «не даёт qty_done
   * уйти выше qty позиции (клампится)» и закреплял ровно тот дефект, о котором
   * пишет заказчик: «заказ 100 шт, фактически выполнено 105 шт — система
   * фиксирует только 100 шт».
   *
   * Теперь факт сохраняется целиком, а этап всё так же закрывается: условие
   * `qty_done >= тираж` при перевыполнении истинно тем более.
   */
  it('факт сверх тиража сохраняется целиком и закрывает этап', async () => {
    seed({ qty_done: 400 });
    await useErpStore.getState().reportProgress('st1', 900);
    const st = getStage();
    expect(st.qty_done).toBe(1300);
    expect(st.status).toBe('done');
  });
