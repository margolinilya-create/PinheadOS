...
describe('страж этапов знает про исполнителя', () => {
  ...
  it('к подрядному этапу проверка «свой ли цех» не применяется', () => {
    ...
    /**
     * Проверка цеха осталась для НАШИХ этапов — второй веткой того же
     * условия. С правки 20.09 (п. 5) её минует и принудительное завершение:
     * разбирает последствия обновления директор, а он не состоит ни в одном
     * цехе. Само `v_force` — право И метка, которую ставит только RPC
     * (см. `serverPermissions.test.ts`), так что сквозным пропуском оно
     * не работает.
     */
    expect(GUARD).toMatch(
      /elsif not v_moving and not v_force[\s\S]{0,60}not public\.erp_can_act_in_dept/,
    );
  });
