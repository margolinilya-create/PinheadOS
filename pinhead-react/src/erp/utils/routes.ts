...
/* ОТК: отдельного участка контроля в структуре производства больше нет
   (правки заказчика 10.08) — в маршрут он не добавляется, сам цех
   деактивирован. Константы `QC_DEPT_CODE` здесь тоже больше нет: её докстрока
   обещала, что «по ней узнают старые этапы и сид справочника», а на деле её
   не звал никто — код `'qc'` живёт в справочнике участков
   (`data/departments`), там же, где все остальные. Объяснение, закрывающее
   вопрос вместо проверки, хуже отсутствующего. */
