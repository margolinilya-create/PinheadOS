...
const BASE_CHAIN: Record<ProductionType, string[]> = {
  ...
  /**
   * experimental вынесен в отдельный модуль «Эксперим. цех» (правка 6) — не этап
   * позиции.
   *
   * ВТО У ОБРАЗЦА НЕТ (правка заказчика 01.09, п. 4): «ВТО снова появилось
   * в маршруте, хотя для экспериментального цеха этого этапа нет. Мы его
   * специально убирали». Цепочка `samples` была дословной копией `sewing`,
   * и ВТО приезжало образцу заодно — даже позиции без единого нанесения.
   * У серии ВТО остаётся: убрано ровно там, где его быть не должно.
   */
  samples: ['supply', 'cutting', 'sewing'],
  outsource: ['supply'],
};
