...
/**
 * Подпись этапа: ОПЕРАЦИЯ, если она названа, иначе имя цеха.
 *
 * У подрядного этапа цех означает «чей это участок ответственности», а человек
 * хочет прочитать, что именно делает подрядчик («Сублимация», а не «Цех ДТФ»).
 *
 * С 12.09 условие «только подрядный» СНЯТО, и это возврат к объявленному
 * смыслу поля: `erp_item_stages.operation` описан как «имя операции, когда оно
 * расходится с именем цеха-владельца», без оговорок про исполнителя. Сужение
 * мешало ровно там, где поле впервые понадобилось нашему цеху: у позиции
 * с вышивкой этапов этого участка два — разработка программы и сама вышивка, —
 * и без подписи они выглядели бы в очереди одинаково («Вышивка» дважды),
 * то есть человек не понял бы, какую задачу берёт.
 */
export function stageLabel(
  stage: Pick<ErpItemStage, 'executor' | 'operation'>,
  departmentName: string,
): string {
  if (stage.operation?.trim()) return stage.operation.trim();
