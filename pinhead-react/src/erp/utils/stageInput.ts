...
/**
 * ВЫХОД ЭТАПА — ВЕСЬ ФАКТ, ВКЛЮЧАЯ ПЕРЕВЫПОЛНЕНИЕ (правка 12.09, п. 5).
 *
 * «При передаче результата на следующий этап не отбрасывать превышение: если
 * предыдущий этап завершён с фактом 105 шт, следующий этап должен получить
 * 105 шт в работу, при этом количество заказа остаётся 100 шт».
 *
 * Прежде здесь стояла обрезка по тиражу, и она молча съедала «плюс» ровно
 * там, где он нужен: закрой сдал 105, а швейка видела «принято 100» и не
 * могла отчитаться за пять лишних изделий. Правило считает `utils/stageQty` —
 * одно на все поверхности, чтобы «факт» на экране цеха и «принято» на входе
 * следующего не разошлись.
 *
 * Закрытый этап по-прежнему считается сданным целиком, если число не набивали:
 * цех мог закрыть его кнопкой «Готово» или переносом на канбане. То же
 * допущение, что в `utils/progress`.
 */
function stageOutput(stage: InputStage, itemQty: number): number {
  return stageFactQty(stage, itemQty);
}
