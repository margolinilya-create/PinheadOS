 ...
 *
 * Его импортируют и оболочка (бейдж меню), и экран, и слайс материалов.
 * Вторая копия любого правила отсюда — это ровно тот способ, которым дефект
 * и появился; по этой же причине модуль перестал быть листом 04.09 и берёт
 * годность материала у `routes.isMaterialPending` вместо собственной формулы
 * по одной колонке (см. `isMaterialSettled`).
 */
...
/**
 * Материал «на месте»: пришёл И ПРИНЯТ складом, зарезервирован со склада
 * или не требуется.
 *
 * ОДНО ПРАВИЛО, А НЕ ДВА (обход 04.09). Здесь стояла своя формула по одной
 * колонке — `status in (received, reserved, not_needed)`, — а гейт запуска
 * цеха (`isMaterialPending`) с 22.07 спрашивает ещё и вердикт склада:
 * пришедший закупочный материал годен ТОЛЬКО после `accepted_full`/
 * `accepted_partial`. Две формулы отвечали на один вопрос и разошлись
 * ровно там, ради чего вторая писалась.
 *
 * Цена нашлась на боевой базе: шесть позиций стоят `received` без годной
 * приёмки — среди них «Кулирка 100хб 250гр», принятая с НЕДОСТАЧЕЙ (40 из 42),
 * — и у ВСЕХ ШЕСТИ закупка уже закрыта, потому что здешняя формула считала
 * их «на месте». На заказе 60448 закрой при этом закрыт целиком (75/75, 50/50,
 * 75/75) — гейт завершения этапа спрашивает эту же функцию и пропустил тираж
 * на материале, который тот же цех не смог бы ВЗЯТЬ в работу. Ровно то,
 * от чего предостерегает правило 30.08 «гейт на входе не заменяет гейта
 * на выходе», — только сломалось оно не отсутствием гейта, а второй копией
 * правила.
 */
export function isMaterialSettled(m: MaterialReadiness): boolean {
  return !isMaterialPending(m);
}
 ...
 * записывает недостачу, пересорт или отказ, а закупщик не видел ни вердикта,
 * ни комментария, ни чисел: строка материала показывала только `status`,
 * который приёмка ставит в `received` при ЛЮБОМ исходе.
 *
 * «Принято частично» держится отдельным кодом, а не в общей куче: материал
 * при этом ГОДЕН в производство (решение 22.07 — цех работает тем, что
...
export function materialAcceptanceIssue(
   ...
   * сравнение идёт через `Number`: `"9" < "10"` в JS ложно, и недостача
   * в девять метров из десяти прошла бы как полная поставка.
   */
  const need = Number(m.qty_expected ?? 0);
  const got = Number(m.qty_received ?? 0);
  return need > 0 && got < need ? 'partial' : null;
}

/** Подписи вердикта склада для строки закупки */
export const ACCEPTANCE_ISSUE_LABELS: Record<MaterialAcceptanceIssue, string> = {
  rejected: 'Склад не принял',
  mismatch: 'Пересорт',
  shortage: 'Недостача',
...
export function supplyMaterialSummary(
  ...
  return {
    ...
    problems: list.filter((m) => {
      /**
       * ВЕРДИКТ СКЛАДА — ПЕРВЫМ, ДО ПРОВЕРКИ «НА МЕСТЕ ЛИ» (обход 04.09).
       * «Принято частично» материал НА МЕСТЕ (цех работает тем, что приехало),
       * и ранний выход по `isMaterialSettled` выбросил бы его из проблем,
       * хотя остаток кто-то обязан довезти. Недостача и отказ теперь и так
       * не «на месте», но порядок важен: он не зависит от того, какой
       * из двух признаков сработает.
       */
      if (materialAcceptanceIssue(m)) return true;
