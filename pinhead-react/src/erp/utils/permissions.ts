...
export const DEFAULT_PERMISSIONS: Record<EmployeeRole, ErpPermission[]> = {
   ...
   */
  /**
   * Кладовщику добавлены `stage.take` и `stage.complete` (правки 07.09, п. 9).
   *
   * ЗАПРЕТ СНЯТ, ПОТОМУ ЧТО ИЗМЕНИЛСЯ ВОПРОС. До 07.09 у склада не было
   * ни одного этапа маршрута, и право «завершить этап» ничего не открывало —
   * то есть было декоративным. Теперь у готового изделия с нанесением маршрут
   * начинается с приёмки на складе (`buildRoute`), и без этих двух прав
   * кладовщик получал бы 42501 от `erp_stage_guard` на СВОЁМ этапе — ровно
   * тот отказ, который 12.08 чинили закупщику.
   *
   * Право ограничено цехом: `storekeeper` входит в `DEPT_BOUND_ROLES`, то есть
   * действовать может только на этапах своего участка, а производственных
   * этапов у склада нет.
   */
  storekeeper: [
