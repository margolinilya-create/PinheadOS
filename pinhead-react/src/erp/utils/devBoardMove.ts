...
export type DevMoveRefusal =
  ...
  | 'sequence'
  /**
   * Нанесение делает ОБЩИЙ ЦЕХ, и пока он не закрыл этап, разработка стоит
...
export interface DevMoveInput {
  ...
  hasBranding?: boolean;
  /**
   * Есть незакрытые задачи нанесений — общий цех ещё работает.
   *
   * «Если нанесение выполняется обычным производственным цехом,
   * экспериментальный цех должен ждать его фактического завершения». Признак
   * считается ВНЕ функции и ровно той же формулой, что серверный автопереход
   * `erp_dev_branding_advance`: задача вне ('done','cancelled'). Её статус
   * ведёт триггер от статуса ЭТАПА, то есть это и есть «цех фактически
   * закрыл» — второго источника правды не заводится.
   */
...
export function devMoveIntent(input: DevMoveInput): DevMoveResult {
  ...
  /**
   * НАНЕСЕНИЯ ДЕЛАЕТ ОБЩИЙ ЦЕХ (правка 01.09, вторая итерация, п. 1).
   *
   * «Сейчас экспериментальный цех даёт вручную перенести карточку с этапа
   * Нанесения в Пошив, даже если нанесение в общем производственном цехе ещё
   * не завершено… Так быть не должно».
   *
   * Держится ВЫХОД, а не вход: карточка обязана попасть в «Нанесения», иначе
   * работа в цех не уедет вовсе. Назад по-прежнему можно — проверка стоит
   * после `isForward`.
   */
  if (from === 'branding' && input.brandingOpen) {
    return { ok: false, reason: 'branding' };
  }
  /**
...
export interface DevMovePrompt {
  ...
  title: string;
  label: string;
  /** Подпись кнопки подтверждения: она зависит от того, откуда идёт карточка */
  confirmLabel: string;
...
export function devMovePrompt(
  ...
  /**
   * СПРАШИВАЕМ ПО СОСТОЯНИЮ ПОЛЯ, А НЕ ПО МЕСТУ В МАРШРУТЕ (правки 02.09, п. 4).
   *
   * «Поле не заполняется повторно. Значение автоматически подтягивается
   * из этапа „Построение лекал“, где техническое название уже было введено».
   * Колонка и так одна (`erp_experimental.pattern_tech_name`), и форма
   * финального пакета её подставляет — то есть «подтягивается» работало.
   * Не работало другое: значения могло не оказаться вовсе.
   *
   * Вопрос был привязан к ходу ВПЕРЁД С ЛЕКАЛ, и это держалось на допущении,
   * что карточка обязательно пройдёт этот шаг руками. Допущение отказало:
   * `moveDevStage` берёт шаг-источник из ХРАНИМОЙ колонки, а доска рисует
   * вычисленную — и разработка, у которой первый ручной перенос сделан
   * с более позднего шага, вопроса не получала НИКОГДА. На проде такая есть:
   * карточка стоит в «Финальном этапе» с пустым названием.
   *
   * Условие «ход вперёд» остаётся: назад карточку двигают, когда работу
   * переделывают, и требовать там результат незачем. А вперёд идёт только тот,
   * у кого построение лекал позади, — то есть спрашивать есть о чём на любом
   * шаге.
   */
  if (!isForward(from, to)) return null;
  if (dev.pattern_tech_name) return null;
  /**
   * Заголовок и кнопка зависят от места: «Завершить построение лекал» на
   * переходе «Крой → Нанесения» было бы неправдой — лекала там давно позади,
   * и вопрос задаётся потому, что результат не записали.
   */
  const leavingPatterns = from === 'patterns';
  return {
    field: 'pattern_tech_name',
    title: leavingPatterns
      ? 'Завершить построение лекал'
      : 'Техническое название лекал не записано',
    label: 'Техническое название лекал',
    confirmLabel: leavingPatterns ? 'Завершить и перенести' : 'Записать и перенести',
    initialValue: '',
  };
}

/**
 * КОММЕНТАРИЙ ПО ПРОРАБОТКЕ (правка 13.09, п. 10).
 *
 ...
 * результат этапа… Поле необязательное: если комментария нет, этап можно
 * завершить и перенести без него».
 *
 * ОТДЕЛЬНАЯ ФУНКЦИЯ, А НЕ ВЕТКА `devMovePrompt`. У той вопрос ОБЯЗАТЕЛЬНЫЙ
 * (без технического названия лекал этап объявлен завершённым без своего
 * результата), а здесь — необязательный, и оба могут понадобиться на одном
 * переходе: карточка, у которой название лекал так и не записали, идёт
 * в «Нанесения» с двумя вопросами подряд. Свести их в один нельзя — один
 * блокирует перенос, другой нет, и общее «required» соврало бы про любой
 * из них.
 *
 * `null` — переход комментария не требует. Спрашиваем только ВПЕРЁД и только
 * при входе в «Нанесения»: назад карточку двигают, когда работу переделывают,
 * и результат там записывать не о чем.
 *
 * Документ называет три участка — «Шелкография», «DTF», «Вышивка», — но
 * по шагу доски они все `branding`: какие именно нанесения заведутся, решают
 * `prints` позиции (`devBrandingFromPrints`), а не этот вопрос. Спрашивать
 * отдельно про каждый участок значило бы завести второй источник правды
 * о составе нанесений — ровно тот, от которого отказались 30.08.
 */
export interface DevNotePrompt {
  /** Куда пишется значение — поле `erp_experimental` */
  field: 'branding_note';
  title: string;
  message: string;
  label: string;
  confirmLabel: string;
  /** Уже записанное значение: правится, а не набирается заново */
  initialValue: string;
}

export function devNotePrompt(
  from: DevStage,
  to: DevStage,
  dev: { branding_note?: string | null },
): DevNotePrompt | null {
  if (to !== 'branding') return null;
  if (!isForward(from, to)) return null;
  return {
    field: 'branding_note',
    title: 'Завершить проработку',
    message: 'Необязательно. Комментарий сохранится в карточке разработки '
      + 'и будет виден цеху нанесения в его задании.',
    label: 'Комментарий по проработке',
    confirmLabel: 'Завершить и перенести',
    initialValue: dev.branding_note ?? '',
  };
}
...
export const DEV_MOVE_REFUSAL_TEXT: Record<DevMoveRefusal, string> = {
  ...
  branding: 'Нанесение ещё не завершено цехом. '
    + 'Карточка пойдёт дальше, когда цех закроет своё задание.',
};
