...
describe('приёмка материала: журнал и статус одной транзакцией', () => {
   ...
   */
  /**
   * ДУБЛЬ ОПРЕДЕЛЯЕТ САМА ВСТАВКА (правка 25.08, файл восстановлен 31.08).
   *
   * Проверки ниже переписаны, и вот почему это не «подгонка под зелёный».
   * Прежде идемпотентность держалась на `select exists (…) into v_dup` перед
   * `insert`, и тест сторожил ИМЕННО ЭТУ форму. Между проверкой и вставкой
   * помещались два параллельных вызова очереди: первый вставлял строку,
   * второй проходил проверку до неё и падал 23505 — очередь сообщала об ошибке
   * приёмки, которая на самом деле прошла. Окно закрыто атомарным
   * `on conflict … do nothing`, и сторожить нужно новую механику: старые
   * строки, оставленные «на всякий случай», требовали бы возврата гонки.
   *
   * Намерение при этом не изменилось ни на букву: повтор той же попытки
   * не удваивает приход. Сумма журнала — это количество материала на фабрике,
   * и обрыв ответа при закоммиченной приёмке (обычное дело на цеховом Wi-Fi)
   * не должен превращаться во второй приход. Без этого нельзя делать
   * и офлайн-очередь: она по определению повторяет отправку.
   */
  it('повтор с тем же ключом не пишет вторую строку журнала', () => {
    const body = withoutComments(functionBody(ACCEPT_SQL, 'erp_material_accept'));
    // Дубль отсекает сама вставка — по частичному уникальному индексу
    expect(body).toMatch(/on conflict \(client_key\) where client_key is not null do nothing/);
    // …и узнаётся по числу вставленных строк, а не по предварительному запросу
    expect(body).toMatch(/get diagnostics v_rows = row_count/);
    expect(body).toMatch(/v_dup := \(v_rows = 0\)/);
    // Предварительной проверки быть НЕ должно: её возврат вернул бы и гонку
    expect(
      body,
      'вернулась проверка перед вставкой — между ней и insert снова помещается второй вызов',
    ).not.toMatch(/select exists/);
    // Статус позиции приводится к запрошенному и при повторе: повтор обязан
    // оставить систему в том же состоянии, а не в половинчатом
  ...
  /**
   * Прежняя сигнатура без ключа снимается: оставленная рядом, она стала бы
   * вторым путём записи журнала — без идемпотентности.
   *
   * ИЩЕТСЯ ПО ВСЕМ МИГРАЦИЯМ, а не в последней, определяющей функцию.
   * Раньше тест смотрел в `ACCEPT_SQL` (последнее определение) и работал
   * ровно до тех пор, пока функцию не пересоздали снова: `drop` относится
   * к миграции, ДОБАВИВШЕЙ параметр (22.08), а более поздняя правка тела
   * его не повторяет и повторять не должна. Привязка к «последней»
   * превращала верное утверждение в ложное падение — и это ровно тот случай,
   * который вскрылся при восстановлении файла 25.08.
   */
  it('сигнатуры без ключа не остаётся', () => {
    latestMatching(
      /drop function if exists public\.erp_material_accept\([\s\S]{0,200}\)/,
      'снятие прежней сигнатуры erp_material_accept без p_client_key');
  });
...
describe('у количества принятого ровно один писатель', () => {
   ...
   */
  /**
   * ПРОВЕРЯЕТСЯ ЗАПИСЬ, А НЕ ЛЮБОЕ ОБРАЩЕНИЕ К ТАБЛИЦЕ (уточнено 16.09).
   *
   * Прежняя редакция запрещала `from('erp_material_receipts')` целиком, и это
   * ловило заодно ЧТЕНИЕ — а чтение журнала слайсу понадобилось по прямому
   * требованию документа: окно приёмки изделия подставляет фактически
   * закупленное по размерам, то есть складывает приходы. Запрет на чтение
   * гнал бы этот запрос в другой файл, где он выглядел бы чужим, ничего
   * при этом не защищая: дефект, ради которого сторож поставлен, — ВТОРОЙ
   * ПИСАТЕЛЬ, а не второй читатель.
   *
   * Строгость от уточнения не падает, а растёт: теперь ловится запись в любой
   * форме — `insert`, `upsert`, `update` и `delete`, — а не одно только
   * обращение по имени таблицы.
   */
  it('клиент не пишет строки журнала мимо RPC', () => {
    for (const [name, text] of [
      ['warehouseSlice', WAREHOUSE_SLICE],
      ['materialsSlice', MATERIALS_SLICE],
    ] as const) {
      const clean = withoutJsComments(text);
      for (const op of ['insert', 'upsert', 'update', 'delete']) {
        expect(clean, `${name} пишет журнал приходов (${op}) в обход erp_material_accept`)
          .not.toMatch(new RegExp(`from\\('erp_material_receipts'\\)[\\s\\S]{0,200}\\.${op}\\(`));
      }
    }
  });
});
