...
describe('посадочная по роли', () => {
  ...
  /**
   * Дизайнер (правка 14.09, п. 3) ведёт файлы В КАРТОЧКЕ ЗАКАЗА, поэтому
   * приземляется на список заказов, а не на обзор производства: его цифры
   * («в работе», «загрузка цехов») к работе дизайнера не относятся.
   */
  it('дизайнер приземляется в заказы', () => {
    expect(landingPathForRole('designer', ALL_OPEN)).toBe('/orders');
  });
