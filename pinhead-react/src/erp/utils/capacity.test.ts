...
/**
 * Неделя может лежать в двух месяцах, а дневная мощность зависит от месяца:
 * при одной месячной норме в феврале рабочих дней меньше, чем в марте.
 * Считалось по месяцу ПЕРВОГО дня — неделя на стыке мерилась целиком чужим
 * месяцем, и «Загрузка производства» врала каждый такой раз.
 */
describe('capacityForDates на стыке месяцев', () => {
  const s = { monthly_units: 10000, work_days_per_week: 5 };

  it('неделя внутри месяца считается по своему месяцу', () => {
    const week = ['2026-09-07', '2026-09-08', '2026-09-09', '2026-09-10', '2026-09-11'];
    const perDay = dailyCapacity(s, '2026-09-01')!;
    expect(capacityForDates(s, week)).toBe(Math.round(perDay * 5));
  });

  it('неделя на стыке считает КАЖДЫЙ день своим месяцем', () => {
    // 31 августа — понедельник; 1–4 сентября — вт–пт
    const week = ['2026-08-31', '2026-09-01', '2026-09-02', '2026-09-03', '2026-09-04'];
    const aug = dailyCapacity(s, '2026-08-01')!;
    const sep = dailyCapacity(s, '2026-09-01')!;
    expect(capacityForDates(s, week)).toBe(Math.round(aug + sep * 4));
  });

  it('месяцы с разным числом рабочих дней дают разную дневную норму', () => {
    // Иначе предыдущий тест ничего не проверяет
    expect(dailyCapacity(s, '2026-08-01')).not.toBe(dailyCapacity(s, '2026-09-01'));
  });

  it('выходные мощности не добавляют', () => {
    const weekend = ['2026-09-05', '2026-09-06'];
    expect(capacityForDates(s, weekend)).toBe(0);
  });
});
