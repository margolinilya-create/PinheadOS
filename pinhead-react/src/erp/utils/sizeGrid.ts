 ...
 *
 * ПОВТОРЯЮЩАЯСЯ ПАРА «ЦВЕТ × РАЗМЕР» СХЛОПЫВАЕТСЯ СУММОЙ (правка 21.09, п. 8).
 *
 * В сетке позиции встречаются ДВЕ строки с одним цветом: форма заказа
 * позволяла добавить второй ряд, не спросив, и на бою 21.09 таких позиций
 * несколько («тест готовое 2» — `[{«—», 3XS:100}, {«—», 3XS:122}]`). Ячейка
 * там адресуется ключом «цвет × размер», а ключ у обеих строк ОДИН — и всё,
 * что построено на этом ключе, ломалось молча:
 *
 *   · таблица ввода рисовала две строки, читающие одно значение, а обратная
 *     сборка (`cellsToGrid`) их складывала — каждое нажатие клавиши удваивало
 *     число («ввёл 1, получил 336842, итог 673684» из документа);
 *   · «в заказе XS 50» показывало первую строку вместо суммы (в заказе 200).
 *
 * Схлопывание здесь, а не у каждого читателя: `gridCells` — единственный вход
 * в сетку для закупки, приёмки, закроя и пошива, и пять одинаковых дедупликаций
 * рядом разошлись бы на первой правке. Семантика та же, что у `cellsToGrid`
 * ниже: повтор — это СУММА, а не «побеждает последний»; тираж позиции при
 * схлопывании не меняется (`erp_size_grid_total` на сервере тоже суммирует всё).
 *
 * Порядок остаётся прежним: цвета — в порядке первого появления, размеры
 * внутри цвета — по шкале.
 */
export function gridCells(grid: SizeGridRow[] | null | undefined): SizeCell[] {
  /** Ключ «цвет × размер» → индекс уже собранной ячейки */
