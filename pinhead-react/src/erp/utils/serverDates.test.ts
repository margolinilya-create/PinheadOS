...
describe('приёмка подряда датируется днём цеха', () => {
  ...
  it('остальная логика приёмки не тронута', () => {
    ...
    // Идемпотентность вставки упаковки с 20.08 держится на проверке
    // существования: уникальность задач стала частичной, и `ON CONFLICT`
    // по ней не выводится (42P10 при каждом срабатывании).
  ...
  it('приёмка при ЭТАПЕ закрывает свою операцию, а не «готовое изделие»', () => {
    /**
     * Прежняя версия искала строку подряда по `op_type='finished_product'` —
     * то есть у подрядного ЭТАПА маршрута не находила ничего, и возврат
     * от подрядчика не закрывал операцию вовсе.
     */
    expect(BODY).toContain('where stage_id = new.stage_id');
  });
});
