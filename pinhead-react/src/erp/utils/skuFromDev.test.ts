...
describe('erp_sku_from_dev — сервер', () => {
  ...
  /**
   * ПРАВО ПЕРЕЕХАЛО НА `sku.publish` (правка 14.09, п. 6), и это не ослабление:
   * носители совпадают — директор и руководитель производства, — а само право
   * засеяно ТОЙ ЖЕ парой миграций, что и перевод гейта. Иначе кто-то потерял
   * бы доступ молча.
   *
   * Прежняя редакция требовала дословного `catalog.edit` и упала бы на верной
   * правке; переписана на СВОЮ причину — «выпуск артикула гейтится правом
   * на выпуск, а не общим правом справочников».
   */
  it('выпуск артикула гейтится правом на выпуск', () => {
    expect(SQL).toMatch(/erp_has_permission\('sku\.publish'\)/);
    expect(SQL).toMatch(/42501/);
    // Общее право справочников выпуск в прайс больше не открывает
    expect(SQL).not.toMatch(/erp_has_permission\('catalog\.edit'\)/);
  });
