...
describe('подпись этапа', () => {
  ...
  /**
   * СУЖЕНИЕ «ТОЛЬКО ПОДРЯДНЫЙ» СНЯТО 12.09 (п. 2).
   *
   * Прежде тест назывался «у нашего этапа операция подпись не подменяет»
   * и закреплял ограничение доводом «иначе одна и та же работа называлась бы
   * по-разному в очереди и на канбане». Довод остался верным — и именно
   * поэтому подпись теперь ОДНА для обоих исполнителей: наш этап с названной
   * операцией зовётся ею везде, а не только в подряде.
   *
   * Понадобилось это цеху вышивки: у позиции два его этапа — разработка
   * программы и сама вышивка, — и без подписи очередь показывала бы
   * «Вышивка» дважды, не давая понять, какую задачу берёшь.
   */
  it('наш этап с названной операцией зовётся операцией', () => {
    expect(stageLabel({ executor: 'internal', operation: 'Разработка программы вышивки' }, 'Вышивка'))
      .toBe('Разработка программы вышивки');
    // Без операции — по-прежнему имя цеха, и это главный случай
    expect(stageLabel({ executor: 'internal', operation: null }, 'Вышивка')).toBe('Вышивка');
  });
});
