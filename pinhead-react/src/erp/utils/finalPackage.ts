...
export function missingFinalPackage(
  ...
  /**
   * СНЯТА ТОЛЬКО «ПРОВЕРКА ОБРАЗЦА» (правка заказчика 12.09, вторая порция,
   * п. 5): «убрать из рабочего сценария только блок „Проверка образца".
   * „Финальный технический пакет" оставить обязательным условием завершения
   * разработки».
   *
   * ЧТО БЫЛО НЕ ТАК. Правка 12.09 сняла ОБА требования разом — одним флагом
   * `devFinalPackageRequired`, который гасил и блок проверки образца,
   * и весь перечень недостающего. Разработку стало можно завершить с пустым
   * техпакетом: ни названия лекал, ни версии, ни паспорта, ни фото. Флага
   * больше нет — требование вернулось насовсем, а не переключателем.
   *
   * УСЛОВИЯ ПО ОБРАЗЦУ (`sample_approved_at`) здесь НЕТ и не будет: вход,
   * которым его проставляли, убран из сценария, и требование поля, которое
   * некому заполнить, заперло бы завершение разработки навсегда. Колонка
   * и её значения остаются — это история уже принятых образцов.
   */
