...
/**
 * ТРЕБУЕТСЯ ЛИ ФИНАЛЬНЫЙ ПАКЕТ (правка заказчика 12.09, п. 9).
 *
 * Отдельная функция, а не чтение флага по месту: читателей четверо — перечень
 * недостающего, форма пакета, карточка шага маршрута и блок «Проверка
 * образца», — и четыре обращения к `FEATURES` разошлись бы в первую же правку.
 *
 * Нужна она именно ПОВЕРХНОСТЯМ: пустой перечень недостающего сам по себе
 * означает «всё собрано», и подпись «Пакет собран» на незаполненных полях
 * была бы неправдой ровно там, где человек ждёт подтверждения.
 */
export function isFinalPackageRequired(): boolean {
  return FEATURES.devFinalPackageRequired;
}
...
export function missingFinalPackage(
  ...
  /**
   * ТРЕБОВАНИЯ ВРЕМЕННО СНЯТЫ (правка заказчика 12.09, п. 9): «временно убрать
   * из рабочего сценария блок „Проверка образца" и обязательность „Финального
   * технического пакета"… функционал можно скрыть/отключить, чтобы его можно
   * было вернуть позже».
   *
   * Ранний выход, а не удаление проверок: флаг `devFinalPackageRequired`
   * возвращает их целиком, без релиза, и перечень остаётся ровно тем же —
   * второй редакции правила, которая разойдётся с этой, не заводится.
   * Серверный страж `erp_dev_package_guard` ослаблен ТЕМ ЖЕ коммитом: клиент,
   * разрешающий то, что сервер отклонит, — это «кнопка есть, действие падает».
   *
   * ЦЕНА НАЗВАНА ВСЛУХ. На переходе в «Готово к серии» висит
   * `erp_dev_handoff_to_warehouse`, заводящий складскую задачу «Приёмка ГП».
   ...
   * Поэтому это отключение, а не снос: вернуть требование — один переключатель.
   */
  if (!isFinalPackageRequired()) return out;
