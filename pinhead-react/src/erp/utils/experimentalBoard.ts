...
/**
 * ОБЩИЙ ЦЕХ ЕЩЁ НЕ ЗАКРЫЛ НАНЕСЕНИЯ — условие, по которому с шага «Нанесения»
 * не выпускают вперёд (правка 01.09, вторая итерация, п. 1).
 *
 * ФОРМУЛА ДОСЛОВНО ПОВТОРЯЕТ СЕРВЕРНУЮ `erp_dev_branding_advance`: задача
 * нанесения вне ('done', 'cancelled'). Статус такой задачи ведёт триггер
 * от статуса ЭТАПА, поэтому это и есть «цех фактически закрыл», а не отдельное
 * мнение доски.
 *
 * ЗАЧЕМ ФУНКЦИЯ, А НЕ ВЫРАЖЕНИЕ НА МЕСТЕ. Читателей формулы три — доска
 * (`Experimental.brandingOpenByDev`), страница разработки и карточка, — и две
 * из трёх копий уже разошлись: страница проверяла только `!== 'done'`,
 * то есть считала ОТМЕНЁННУЮ задачу нанесения незакрытой. Один и тот же
 * образец на доске выпускался в «Пошив», а на своей странице — нет.
 * Расхождение молчаливое: обе половины «работают», просто отвечают разное.
 */
export function devBrandingOpen(
  tasks: readonly { task_type?: string | null; status?: string | null }[] | null | undefined,
): boolean {
