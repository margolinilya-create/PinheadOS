...
export function validateOrderForm(
   ...
   */
  /**
   * СРОК КЛИЕНТА ОБЯЗАТЕЛЕН (аудит 02.09.2026).
   *
   * Поле было необязательным, и на проде срок стоял лишь у 6 из 28 активных
   * заказов. Слепыми от этого становятся сразу три механизма: просрочка
   * заказа (`isOrderOverdue` сравнивать не с чем), подстановка плановой даты
   * этапа (`defaultPlannedEnd` берёт срок заказа и без него ставит «сегодня»,
   * делая этап просроченным назавтра) и закрытие заказа — `erp_ship_order`
   * до этой же правки объявлял заказ без срока сданным вовремя.
   *
   * Идёт в `missing`, а не в `invalid`: поле не заполнено, а не заполнено
   * неверно, — от этого зависит текст подсказки у кнопки.
   */
  if (!form.due_date) {
    errors.due_date = 'Укажите срок клиента';
  ...
  } else if (form.launch_date && form.due_date < form.launch_date) {
    /**
