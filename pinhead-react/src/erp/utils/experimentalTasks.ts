...
/**
 * Типы задач нанесения, ВСТРЕЧАЮЩИЕСЯ в разработке (для повтора того же
 * участка). `dtg` остаётся, хотя участок снят 07.09 (п. 17): на боевой базе
 * есть заведённая задача этого вида, и доработка по областям обязана её
 * повторить — набор отвечает «что уже было», а не «что предлагаем».
 */
