...
describe('gridCells', () => {
  ...
  /**
   * ДУБЛИ СХЛОПЫВАЮТСЯ СУММОЙ (правка 21.09, п. 8).
   *
   * Данные не выдуманы: у позиции «тест готовое 2» на бою лежит ровно
   * `[{«—», 3XS:100}, {«—», 3XS:122}]`. Две ячейки с одним ключом «цвет ×
   * размер» давали таблицу, где оба поля читают одно значение, а обратная
   * сборка их складывает: каждое нажатие клавиши удваивало введённое.
   */
  it('две строки одного цвета с одним размером — одна ячейка на сумму', () => {
    expect(gridCells([
      { color: '—', sizes: { '3XS': 100 } },
      { color: '—', sizes: { '3XS': 122 } },
    ])).toEqual([{ color: '—', size: '3XS', qty: 222 }]);
  });

  it('склейка не меняет тираж позиции', () => {
    const withDup = [{ color: '—', sizes: { XS: 50 } }, { color: '—', sizes: { XS: 150 } }];
    expect(gridRowsTotal(withDup)).toBe(200);
  });

  it('пустой цвет и прочерк — один и тот же цвет, а не два', () => {
    expect(gridCells([
      { color: '', sizes: { S: 1 } },
      { color: '—', sizes: { S: 2 } },
    ])).toEqual([{ color: '—', size: 'S', qty: 3 }]);
  });

  it('разные размеры одного цвета остаются разными ячейками', () => {
    expect(gridCells([
      { color: '—', sizes: { XS: 5 } },
      { color: '—', sizes: { S: 7 } },
    ])).toEqual([
