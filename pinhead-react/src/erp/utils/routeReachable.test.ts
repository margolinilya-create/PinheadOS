 ...
 *
 * ЧТО СЛУЧИЛОСЬ. Заказ с маршрутом «Закупка → Закрой → ДТФ → Швейный цех → ВТО»
...
const OWN_SCREEN: Record<string, string> = {
  ...
  /**
   * Приёмка готового изделия (правки 07.09, п. 9): у готового изделия
   * с нанесением маршрут начинается со склада. Блок стоит НАД списком
   * складских задач и читает ЭТАПЫ (`buildQueueEntries`), а не задачи —
   * иначе экран снова выглядел бы рабочим, спрятав заказ целиком.
   */
  warehouse: 'erp/screens/warehouse/FgIntakeQueue.jsx',
};
