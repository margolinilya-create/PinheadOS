 ...
 * по ВСЕЙ фабрике: ограничение по цеху на человека без цеха для СКВОЗНЫХ ролей
 * не действует и сейчас (`canActInDept` для них fail-open — цеха нет ни
 * у менеджера, ни у диспетчера), а `manager` как раз сквозная. Ролям участка
 * пустая привязка с 22.08 закрывает всё: у них есть права на этапы, и матрица
 * их не держит. Одна галочка «Подтвердить» в админке выдавала весь набор.
 *
