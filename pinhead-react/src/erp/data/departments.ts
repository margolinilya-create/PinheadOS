 ...
 *
 * Ключ ПЕРЕБИВАЕТ имя участка из базы (`DEPT_SHORT_NAMES[code] || fallback`),
 * поэтому здесь перечислены только те участки, чьё полное имя в плотной
 * строке не помещается. Участка, которому короткое имя не нужно, в словаре
 * быть не должно: иначе одно название живёт в двух местах и однажды
 * разъезжается с базой.
 *
 * Так и вышло со «Швейкой» (правка заказчика 12.09, п. 6): в `erp_departments`
 * участок `sewing` называется «Швейный цех» с самой первой миграции, а на всех
 * экранах печаталось «Швейка» — из этой строки. Ключ снят, имя приходит
 * из данных.
 */
...
/**
 * Цеха с рабочей очередью (очередь участка, загрузка на дашборде).
