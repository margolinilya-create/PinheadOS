...
describe('CSS-модули раздела «Производство»', () => {
   ...
   */
  /*
    ЗАПАС ПО ВРЕМЕНИ, А НЕ ПРОПУСК (правка 22.09). Тест разбирает КАЖДЫЙ файл
    раздела и каждый его CSS-модуль: его время растёт вместе с разделом,
    а не зависит от проверяемого кода. Локально он укладывается в 1.8 с при
    бюджете по умолчанию 5 с — меньше трёхкратного запаса, и на CI, где рядом
    идут ещё 277 файлов на общих ядрах, он в этот бюджет не уложился
    (прогон 22.09, «Test timed out in 5000ms»).

    Такой отказ ничего не сообщает о коде: он краснеет от загрузки раннера.
    Сторож, который «иногда красный», — сторож, мимо которого смотрят,
    поэтому запас поднят до двадцати секунд. Проверять он продолжает ровно
    то же самое: ни одного `it.skip`, ни одной снятой проверки.
  */
