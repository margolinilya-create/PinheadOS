 ...
 *
 * Приём с bencho.dev (блок `Pull to refresh`): порог, а не инерция. Полоса
 * не «доезжает» сама и ничего не предсказывает — есть ход пальца и есть
 * черта, за которой жест считается состоявшимся.
 *
 * ─── НО КОММИТ ЗДЕСЬ НА ОТПУСКАНИИ, А НЕ НА ПОРОГЕ ────────────────────────
 * И это осознанное расхождение с `SlideConfirm`, где сделано наоборот.
 * У протяжки подтверждения порог — это ЗАЩИТА: довести и ещё отпустить
 * значило бы дать два способа не сработать там, где человек уже решился.
 * Здесь наоборот: порог легко перейти СЛУЧАЙНО, пролистывая список сверху,
 * и возможность передумать — вернуть палец вверх и отпустить — дороже
 * мгновенности. Сетевое действие, которого не просили, хуже лишней
 * десятой доли секунды.
 *
 * Поэтому же полоса уезжает обратно при обратном ходе, а сам жест
 * не отменяется: человек передумал, а не ошибся экраном.
 *
