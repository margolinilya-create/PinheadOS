...
const EMBEDS: Record<string, string[]> = {
  /**
   * `rolls` — рулоны партии (правка 16.09, п. 5): своя таблица
   * `erp_material_rolls`, а не колонка. Подмешивается вложенным select-ом
   * туда, где их выбирает закройщик; отдельный запрос на каждую строку
   * закупки был бы N+1.
   */
  ErpMaterial: ['suppliers', 'rolls'],
...
describe('ручные типы ERP не разошлись со схемой БД', () => {
  ...
  it('список отношений не разросся: каждое должно быть осознанным', () => {
    ...
    // 16.09: четвёртым стал `rolls` — рулоны партии материала (правка п. 5)
    expect(Object.values(EMBEDS).flat()).toHaveLength(4);
  });
