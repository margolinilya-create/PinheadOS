...
- Одна UPDATE-операция под разными правами разделяется ТРИГГЕРОМ по изменившимся
  колонкам (`erp_calendar_guard`), а не политикой: RLS работает на уровне строки.
  Пустой `auth.uid()` в таком страже — это service_role, его пропускаем: он и так
  минует RLS, и запирать починку через SQL нельзя
