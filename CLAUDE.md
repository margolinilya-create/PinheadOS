...
## ⚠️ Активные ветки (2026-04-13)

| Ветка | Назначение | Статус | Deploy |
|---|---|---|---|
| `main` | **Production** — менеджеры работают здесь | STABLE, не ломать | `pinhead-os.vercel.app` |
| `redesign/v2` | **Production-слой редизайн** (3 месяца, W1 старт) | active dev | Vercel preview URL |
...
### redesign/v2 основные добавления

- **Production-слой** (Tech Card Builder → Workshop Board → Мастер-экран бригадира)
- 7 bounded contexts (Sales / Catalog / TechDesign / Scheduling / Execution / QC / Payroll) — отдельные Zustand root stores (ADR-0001)
- Domain events outbox через pg_cron dispatcher (ADR-0004, ADR-0008)
- Rate + minutes snapshot на `order_tech_operations` (ADR-0002)
- `tenant_id` с day 1 для multi-tenant готовности (ADR-0003)
- DB-level invariants: `paid_at` immutability trigger, `amount = rate × qty` CHECK, `order_id` denorm consistency trigger (миграция 20260510)
- Sub_role для production (`foreman` / `senior_foreman` / `technologist` / `procurement` / `qc_operator`)
- `piecework` parallel-run обязательный перед cutover (ADR-0007)

См. `docs/adr/` для всех решений.
