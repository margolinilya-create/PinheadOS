...
## ⚠️ Активные ветки (2026-04-14)

| Ветка | Назначение | Статус | Deploy |
|---|---|---|---|
| `main` | **Production** — менеджеры работают здесь | STABLE, не ломать | `pinhead-os.vercel.app` |
| `redesign/v2` | **Production-слой редизайн** | **W1+W2+W3+W4 ЗАКРЫТЫ**, UAT round 1+2 пройден, готов к директорскому UAT + cutover. Bitrix sync блокирован. | Vercel preview → v2 Supabase |
...
**Закрыто (W1-W4 + UAT round 1+2):**
- 8 миграций применены к v2 Supabase (последняя `20260525_piecework_update_policy_fix`)
- 6 ADR-0001 root stores: useTechCardStore, useWorkshopStore, useForemanStore, useWorkersStore (+ restore), usePayrollStore, useNotificationsStore + useUndoStore
- 8 production routes под feature flags: `/orders/table` `/tech-cards` `/tech-cards/:id` `/workshop` `/foreman` `/payroll` `/workers` `/trash`
- NotificationsBell + V2Nav (sticky, z-index выше overlay) + UndoToastHost (floating widgets)
...
- piecework reversal flow для closed batches
- **UAT round 1+2:** blocker (payroll close RLS policy) + 3 major (confirm dialogs) + minor (i18n, onboarding z-index) — всё починено, round 2 PASS по всем пунктам
- Director UAT checklist: `docs/UAT-director-checklist.md`
- 846/846 тестов зелёные
